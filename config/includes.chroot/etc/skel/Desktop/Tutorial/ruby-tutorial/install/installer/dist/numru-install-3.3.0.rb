...
$Id: numru-install.rb,v 1.28 2004/12/22 03:56:05 koshiro Exp $

Change Log:

* 2004/12/22 KOSHIRO Tsuyoshi <koshiro at gfd-dennou dot org>
  - (3.3.0)
  - deal with changing of the tarball name and URL of dcl-C.
  - deal with changing the installation script of gave.

* 2004/08/12 KOSHIRO Tsuyoshi <koshiro at gfd-dennou dot org>
  - (3.2.1)
  - bug fix.
...
* 2004/08/11 KOSHIRO Tsuyoshi <koshiro at gfd-dennou dot org>
  - (3.1.0)
  - change class RubySoft: deal with changing installation script of gave.
  - name of package is changed: misc -> numru-misc
  - bug fix.

* 2004/07/15 KOSHIRO Tsuyoshi <koshiro at gfd-dennou dot org>
  - (3.0.0)
...
* 2003/12/18 KOSHIRO Tsuyoshi <koshiro at gfd-dennou dot org>
  - (2.0.1)
...
* 2003/12/10 KOSHIRO Tsuyoshi <koshiro at gfd-dennou dot org>
  - (2.0.0)
...
* 2003/06/27 KOSHIRO Tsuyoshi <koshiro at gfd-dennou dot org>
  - (1.0.0)
  - first release.
	...
	module Install
		...
		class Software
			...
			def install(dl_dir = DefaultDownloadDir, prefix = DefaultPrefix)
				...
				pdir = @name == "netcdf" ? "../.." : ".."
				if @name == "netcdf" && RUBY_PLATFORM =~ /linux/
		...
		class RbLib < RubyLib
			def install(dl_dir = DefaultDownloadDir, rbsitelibdir = DefaultRbSiteLibDir, prefix = DefaultPrefix)
				...
				if @name == "narray_miss"
					raise error_msg unless system("ruby setup.rb config --site-ruby=#{rbsitelibdir}")
					raise error_msg unless system("ruby setup.rb install")
				else
				...
				if @name == "gphys"
					...
					raise error_msg unless system("sed -e 's/bindir = .*$/bindir = \"#{esc_prefix}\\/bin\"/' install.rb.orig > install.rb")
					raise error_msg unless system("/bin/rm -f install.rb.orig")
					raise error_msg unless system("ruby install.rb")
				end
				Dir::chdir(cur_dir)
				self.save_latest_ver if @current_ver_method != "ModuleConst"
				add_rb_load_path(rbsitelibdir)
