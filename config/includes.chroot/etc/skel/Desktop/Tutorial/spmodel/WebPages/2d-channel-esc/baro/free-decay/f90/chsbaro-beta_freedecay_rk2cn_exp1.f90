...
!
program chsbaro_beta_freedecay_rk2cn_exp1
...
!== 宣言部 ============================================================
 ...
 !---- NAMELIST 変数 ----
  ...
  character(len=100)  :: nmlfile='chsbaro-beta_freedecay_rk2cn_exp1.nml'
 ...
 !---------------- NAMELIST 読み込み ---------------------
  ...
  if ( size(argv) .le. 0 ) then
     call MessageNotify('W','chsbaro-beta_rk2cn_exp1',&
                            'There is no argument. The defalut namlist file, '&
                            //trim(nmlfile)//' is used.')
  else
     nmlfile=argv(1)
     call MessageNotify('M','chsbaro-beta_rk2cn_exp1',&
 ...
 !
  subroutine output_restart_init
    call HistoryCreate( &
           file=trim(rst_file), &
           title=trim(title), &
           source='plbaro-beta_freedecay_rk2cn_exp1.f90 (2008/10/16)', &
 ...
 !
  subroutine output_history_init
    ...
    call HistoryCreate( &
           file=trim(hst_file), &
           title=trim(title), &
           source='chsbaro-beta_freedecay_rk2cn_exp1.f90 (2008/10/16)', &
           institution='GFD_Dennou Club SPMODEL project',     &
           dims=(/'x','y','k','l','t'/), &
           dimsizes=(/im,jm+1,2*km+1,lm,0/),&
           longnames=(/'X           ','Y           ',&
 ...
 !
  subroutine output_history
   ...
   !---- 診断量出力 ----
    ...
    call HistoryPut('pv',transpose(yx_Vor+Beta*yx_Y))
...
end program chsbaro_beta_freedecay_rk2cn_exp1
