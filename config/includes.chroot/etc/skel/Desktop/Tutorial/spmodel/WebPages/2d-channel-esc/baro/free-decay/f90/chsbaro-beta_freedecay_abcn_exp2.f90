...
!       Adams-Bashforth scheme and Clank-Nicolson scheme. 
...
!
program chsbaro_beta_freedecay_abcn_exp2
...
!== 宣言部 ============================================================
 ...
 !---- 変数(スペクトルデータ) ----
  ...
  real(8), allocatable :: es_DVorDt1(:,:)     ! 拡散以外の渦度時間変化(t)
  real(8), allocatable :: es_DVorDt0(:,:)     ! 拡散以外の渦度時間変化(t-Δt)
 ...
 !---- NAMELIST 変数 ----
  ...
  character(len=100)  :: nmlfile='chsbaro-beta_freedecay_abcn_exp2.nml'
 ...
 !------------------ 変数の割り付け ---------------------
  ...
  allocate(es_DVorDt1(-km:km,lm),es_DVorDt0(-km:km,lm))
...
&                 c2='Time integration starts.') 
  ...
  es_DVorDt1 = - es_Jacobian_es_es(es_StrFunc,es_Vor)  &
  ...
  do it=1,nstep
     ...
     es_DVorDt0 = es_DVorDt1
     es_DVorDt1 = - es_Jacobian_es_es(es_StrFunc,es_Vor)  &
                  - es_yx(yx_Beta * yx_es(es_Dx_es(es_StrFunc)))

     ! 拡散以外の項を時間積分(Adams-Bashforth scheme)
     es_Vor = es_Vor + delta_t * &
               (3.0d0/2.0d0 * es_DVorDt1 - 1.0d0/2.0d0 * es_DVorDt0)
 ...
 !
  subroutine output_restart_init
    call HistoryCreate( &
           file=trim(rst_file), &
           title=trim(title), &
           source='plbaro-beta_freedecay_abcn_exp2.f90 (2008/10/13)', &
 ...
 !
  subroutine output_history_init
    ...
    call HistoryCreate( &
           file=trim(hst_file), &
           title=trim(title), &
           source='chsbaro-beta_freedecay_abcn_exp2.f90 (2008/10/13)', &
           institution='GFD_Dennou Club SPMODEL project',     &
           dims=(/'x','y','k','l','t'/), &
           dimsizes=(/im,jm+1,2*km+1,lm,0/),&
           longnames=(/'X           ','Y           ',&
...
end program chsbaro_beta_freedecay_abcn_exp2
