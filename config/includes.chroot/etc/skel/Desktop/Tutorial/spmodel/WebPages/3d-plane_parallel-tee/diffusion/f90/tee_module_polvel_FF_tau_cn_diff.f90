...
!
!履歴  2009/12/26  竹広真一: et_FF_cn_diff.f90 より改造
!      2012/06/28  竹広真一: tee_module_polvel_FF_cn_diff.f90 よりタウ法に改造
!      2012/07/06  竹広真一: タウ法計算法変更
!      2012/08/24  竹広真一: 時間積分法変更
!      
!
program tee_module_polvel_FF_tau_cn_diff
  !
   ...
   !------------------- 出力 ----------------------
    subroutine output_gtool4_init
      ...
      call HistoryCreate( &                               ! ヒストリー作成
           file='tee_module_polvel_FF_tau_cn_diff.nc', &
    ...
    subroutine output_gtool4
      ...
      if ( n .ge. 0 ) then
         zyx_PhiSol = zyx_PhiInit &
              * exp( - Pr * (   (2*PI*l/XD)**2 + (2*PI*m/YD)**2 &
                                 +((2*n+1)*PI/ZD)**2   )*time )
      else
         zyx_PhiSol = zyx_PhiInit &
              * exp( - Pr * (  (2*PI*l/XD)**2  + (2*PI*m/YD)**2 &
...
end program tee_module_polvel_FF_tau_cn_diff
