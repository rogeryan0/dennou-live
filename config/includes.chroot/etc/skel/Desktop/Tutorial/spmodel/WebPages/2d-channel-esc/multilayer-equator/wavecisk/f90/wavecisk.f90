...
!    integer, parameter :: im=200,   jm=100, vm = mm+1  ! 格子点の設定(X,Y,Z)
 ...
 !---- 変数 ----
    real(8)            :: yxz_U(0:jm,0:im-1,vm)     ! 格子データ(U(t+Δt))
    real(8)            :: yxz_V(0:jm,0:im-1,vm)     ! 格子データ(V(t+Δt))
    real(8)            :: yxz_H(0:jm,0:im-1,vm)     ! 格子データ(H(t+Δt))
    real(8)            :: yxm_U(0:jm,0:im-1,mm)     ! 格子データ(U(t+Δt))
    real(8)            :: yxm_V(0:jm,0:im-1,mm)     ! 格子データ(V(t+Δt))
    real(8)            :: yxm_H(0:jm,0:im-1,mm)     ! 格子データ(H(t+Δt))
...
!    yxm_H = H0   ! 平均の深さ
    ...
    ! 水平スペクトル変換
    do m = 1, mm 
       ecm_U(:,:,m) = ec_yx(yxm_U(:,:,m) )
       esm_V(:,:,m) = es_yx(yxm_V(:,:,m) )
       ecm_H(:,:,m) = ec_yx(yxm_H(:,:,m) )
    enddo
...
!&                    - ec_dx_ec(ec_yx(yx_ec(ec_H)*yx_ec(ec_U))) &
!&                    - ec_dy_es(es_yx(yx_ec(ec_H)*yx_es(es_V))) &
&                    - eta_m * ec_QH                           &
&                                    )                         &
...
&                    )
     ...
     ! 鉛直モード逆変換 (u,v,phi)
     subroutine inv_vmode_transpote(yxm_data, yxz_data)
       
       real(8),intent(in) :: yxm_data(0:jm,0:im-1,mm)  ! 鉛直モードデータ
       real(8),intent(out):: yxz_data(0:jm,0:im-1,vm)  ! 鉛直格子データ
      
       yxz_data = 0.0 
       do j=1,vm
          do m=1,mm   
             yxz_data(:,:,j) = yxz_data(:,:,j)                  &
                  + ( eig(m,j) - eig(m,j-1) )                   &
                  * yxm_data(:,:,m) * vm / gamma(m) / (2**0.5)       
          enddo
       enddo
 ...
 !----------------------- gtool4 設定 ----------------------------
    ...
    subroutine output_gtool4
      ...
      real(8)  :: yxz_tr_U(0:im-1,0:jm,vm) ! 格子データ (X-Y 軸入れ換え)
      real(8)  :: yxz_tr_V(0:im-1,0:jm,vm) ! 格子データ (X-Y 軸入れ換え)
      real(8)  :: yxz_tr_H(0:im-1,0:jm,vm) ! 格子データ (X-Y 軸入れ換え)
      
      write(6,*) 'it = ',it

      ! 水平スペクトル逆変換
      do m = 1, mm 
         yxm_U(:,:,m) = yx_ec(ecm_U(:,:,m)) 
         yxm_V(:,:,m) = yx_es(esm_V(:,:,m)) 
         yxm_H(:,:,m) = yx_ec(ecm_H(:,:,m)) 
      enddo

      ! 鉛直モード逆変換
      call inv_vmode_transpote(yxm_U, yxz_U)
      call inv_vmode_transpote(yxm_V, yxz_V)
      call inv_vmode_transpote(yxm_H, yxz_H)

      ! X-Y 軸入れ換え
      do m = 1, vm 
         yxz_tr_U(:,:,m) = transpose(yxz_U(:,:,m))
         yxz_tr_V(:,:,m) = transpose(yxz_V(:,:,m))
         yxz_tr_H(:,:,m) = transpose(yxz_H(:,:,m))
      enddo

      call HistoryPut('t',real( it*dt ) )
      call HistoryPut('uvel', yxz_tr_U )
      call HistoryPut('vvel', yxz_tr_V )
      call HistoryPut('height', yxz_tr_H )
      call HistoryPut('wvel', transpose( yx_ec(ec_QH_A)) )
