...
!            +(-1)^{p+1}\nu_{2p}\nabla^2p \nabla^2\psi
!
program plbaro_beta_abcn_test1
...
!== 宣言部 ============================================================
 ...
 !---- 変数(スペクトルデータ) ----
  ...
  real(8), allocatable :: ee_DVorDtB(:,:)     ! 渦度時間変化(t-Δt)
 ...
 !---- NAMELIST 変数 ----
  ...
  character(len=30),parameter  :: nmlfile='plbaro-beta_abcn_test1.nml'
 ...
 !------------------- 初期値設定 ----------------------
  ...
  if ( initial_file == "") then
     ! リスタートファイルを指定しない場合内部で ee_Vor, ee_DVorDtB を与える. 
 ...
 !
  subroutine output_restart_init
    call HistoryCreate( &
           file=trim(rst_file), &
           title=trim(title), &
           source='plbaro-beta_abcn_test1.f90 (2005/10/31)', &
...
end program plbaro_beta_abcn_test1
