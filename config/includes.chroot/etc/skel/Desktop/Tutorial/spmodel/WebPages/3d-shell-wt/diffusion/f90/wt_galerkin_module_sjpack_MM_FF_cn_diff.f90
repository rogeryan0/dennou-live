...
!
!履歴  2009/09/18  竹広真一  wt_galerkin_MM_SS_cn_diff.f90 を sjpack 用に改造
!                            境界条件スイッチ名を S -> F に変更
!
program wt_galerkin_module_sjpack_MM_FF_cn_diff
  !
 ...
 !------------------- 初期値設定 ----------------------
  ...
  if ( l==0 )then
     sigma = 0.0
     write(6,*) 'Radial mode = r '
     write(6,*) 'Growth rate : unknown'
     wz_Torvel = 0.0
     do k=0,km
        wz_Torvel(l_nm(n,m),k) = z_Rad(k)
     enddo
  else
     alpha = CalAlpha(n,l-1)
     sigma = -nu* alpha **2
     ...
     do k=0,km
        wz_Torvel(l_nm(n,m),k) &
         = -(alpha*sbn1(n,ri*alpha)-sbn(n,ri*alpha)/ri)*sbj(n,alpha*z_Rad(k)) &
         +(alpha*sbj1(n,ri*alpha)-sbj(n,ri*alpha)/ri)*sbn(n,alpha*z_Rad(k))
     enddo
  end if
...
contains
  ...
  function det(l,x)
    ...
    det=(x*sbj1(l,ri*x)-sbj(l,ri*x)/ri)*(x*sbn1(l,ro*x)-sbn(l,ro*x)/ro) &
       -(x*sbn1(l,ri*x)-sbn(l,ri*x)/ri)*(x*sbj1(l,ro*x)-sbj(l,ro*x)/ro)
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    ...
    call HistoryCreate( &                                  ! ヒストリー作成
           file='wt_galerkin_module_sjpack_MM_FF_cn_diff.nc', &
...
end program wt_galerkin_module_sjpack_MM_FF_cn_diff
