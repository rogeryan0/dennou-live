...
!
!履歴  2009/12/28  竹広真一: et_RR_cn_diff.f90 より改造
!      2012/06/28  竹広真一: tee_module_polvel_RR_cn_diff.f90 よりタウ法に改造
!      2012/07/06  竹広真一: タウ法計算法変更
!      2012/08/24  竹広真一: 時間積分法変更
!      
!
program tee_module_polvel_RR_tau_cn_diff
  !
 ...
 !---- 物理パラメター ----
  ...
  real(8), parameter          :: ZD=1.0         ! z 方向領域の大きさ
 ...
 !------------------- 初期値設定 ----------------------
  ...
  write(6,*) 'Positvie n:cosh(nz)+cos(nz), negative n: sinh(nz)+sin(nz)'
  write(6,*) 'l,m,n?'
  read(5,*)  l,m,n

  if ( l .ge. 0 .AND. m .ge. 0 .AND. n .ge. 0) then
     write(6,*) 'Type of the initial value : cos(lx)*cos(my)*[cosh(nz)+cos(nz)]'
     rk = sqrt((2*l*Pi/XD)**2+(2*m*Pi/YD)**2)
     rn = InvXtanX(-rk*ZD/2*tanh(rk*ZD/2),abs(n)) * 2/ZD
     zyx_Phi = cos(2*l*PI/XD*zyx_X) * cos(2*m*PI/YD*zyx_Y) &
          * ( cos(rn*ZD/2)* cosh(rk*zyx_Z)- cosh(rk*ZD/2)*cos(rn*zyx_Z) )
  else if ( l .lt. 0 .AND. m .ge. 0 .AND. n .ge. 0) then
     write(6,*) 'Type of the initial value : sin(lx)*cos(my)*[cosh(nz)+cos(nz)]'
     rk = sqrt((2*l*Pi/XD)**2+(2*m*Pi/YD)**2)
     rn = InvXtanX(-rk*ZD/2*tanh(rk*ZD/2),abs(n)) * 2/ZD
     zyx_Phi = sin(2*l*PI/XD*zyx_X) * cos(2*m*PI/YD*zyx_Y) &
          * ( cos(rn*ZD/2)* cosh(rk*zyx_Z)- cosh(rk*ZD/2)*cos(rn*zyx_Z) )
  else if ( l .ge. 0 .AND. m .lt. 0 .AND. n .ge. 0) then
     write(6,*) 'Type of the initial value : cos(lx)*sin(my)*[cosh(nz)+cos(nz)]'
     rk = sqrt((2*l*Pi/XD)**2+(2*m*Pi/YD)**2)
     rn = InvXtanX(-rk*ZD/2*tanh(rk*ZD/2),abs(n)) * 2/ZD
     zyx_Phi = cos(2*l*PI/XD*zyx_X) * sin(2*m*PI/YD*zyx_Y) &
          * ( cos(rn*ZD/2)* cosh(rk*zyx_Z)- cosh(rk*ZD/2)*cos(rn*zyx_Z) )
  else if ( l .lt. 0 .AND. m .lt. 0 .AND. n .ge. 0) then
     write(6,*) 'Type of the initial value : sin(lx)*sin(my)*[cosh(nz)+cos(nz)]'
     rk = sqrt((2*l*Pi/XD)**2+(2*m*Pi/YD)**2)
     rn = InvXtanX(-rk*ZD/2*tanh(rk*ZD/2),abs(n)) * 2/ZD
     ...
     rk = sqrt((2*l*Pi/XD)**2+(2*m*Pi/YD)**2)
     rn = InvXcotX(rk*ZD/2/tanh(rk*ZD/2),abs(n)) * 2/ZD
     zyx_Phi = cos(2*l*PI/XD*zyx_X) * cos(2*m*PI/YD*zyx_Y) &
          * ( sin(rn*ZD/2)* sinh(rk*zyx_Z)- sinh(rk*ZD/2)*sin(rn*zyx_Z) )
...
!!$  zyx_PhiInit = zyx_tee(tee_Phi)
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                                         ! Cn 法による時間積分
     ...
     tee_Phi = tee_LaplaPol2Pol_tee(tee_LaplaPhi,cond=VelBC)
   ...
   !------------------- 拡散項(流線関数用) ----------------------
    subroutine CNDiffusionMatrixPol( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      do n=0,nm
         ...
         tee_DI = tee_LaplaPol2Pol_tee(tee_DI,cond=VelBC)
   ...
   !------------------- 出力 ----------------------
    subroutine output_gtool4_init
      ...
      call HistoryCreate( &                               ! ヒストリー作成
           file='tee_module_polvel_RR_tau_cn_diff.nc', &
...
end program tee_module_polvel_RR_tau_cn_diff
