...
program wu_diff_cn_polvel_R
 ...
 !---- 変数 ----
  real(8), dimension(0:im-1,jm,0:km) :: xyr_Polvel     ! ポロイダル速度場
  real(8), dimension((nm+1)**2,0:lm) :: wu_Polvel      ! ポロイダル速度場
  real(8), dimension((nm+1)**2,0:km) :: wr_Polvel      ! ポロイダル速度場
  real(8), dimension((nm+1)**2,0:lm) :: wu_LaplaPolvel ! ▽^2ポロイダル

  real(8), dimension(0:im-1,jm,0:km) :: xyr_PolvelInit  ! ポロイダル速度場(初期値)
  real(8), dimension(0:im-1,jm,0:km) :: xyr_PolvelSol   ! ポロイダル速度場(解析解)

 ! Crank Nicholson 設定
  real(8)             :: DifLUMT_Polvel((nm+1)*(nm+1),0:lm,0:lm)
                         ! Crank Nicholson 陰的計算用拡散行列(ポロイダル速度場)
  integer             :: kpivot_Polvel((nm+1)*(nm+1),0:lm)
 ...
 !------------------- 初期値設定 ----------------------
  ...
  wu_Polvel = wu_xyr(xyr_Polvel)
  wu_LaplaPolvel = wu_wr(wr_Lapla_wu(wu_Polvel))
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                               ! Crank-Nicolson 法による時間積分
     ...
     wu_LaplaPolvel = wu_LaplaPolvel &
          + dt/2 *( nu * wu_wr(wr_Lapla_wu(wu_LaplaPolvel))  )
     wu_Polvel = wu_LaplaPol2PolGrid_wu(wu_LaplaPolvel,cond='R')
     wu_Polvel = LuSolve(DifLUMT_Polvel,kpivot_Polvel,wu_Polvel)
     wu_LaplaPolvel = wu_wr(wr_Lapla_wu(wu_Polvel))
...
contains
  ...
  function det(l,x)
    ...
    a(2,1) = l*ra**(l-1) ; a(2,2) = x*sbj1(l,ra*x)
...
99  continue
    ...
    subroutine CNDiffusionMatrixPolvel( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      ! Crank Nicholson 設定
      real(8), intent(OUT)  :: DiffLUMatrix((nm+1)*(nm+1),0:lm,0:lm)
                              ! Crank Nicholson 拡散陰的計算用行列
                              ! (1-D dt/2▽^2, LU 分解)

      integer, intent(OUT)  :: kpivot((nm+1)*(nm+1),0:lm)     ! ピボット情報
                              ! Crank Nicholson 拡散陰的計算用行列ピボット情報

      real(8)               :: wu_I((nm+1)*(nm+1),0:lm)       ! 作業用変数
      real(8)               :: wu_DI((nm+1)*(nm+1),0:lm)       ! 作業用変数
      ...
      do l=0,lm
         ...
         wu_DI = - Diffc * dt/2.0 &
                  * wu_wr(wr_Lapla_wu(wu_wr(wr_Lapla_wu(wu_I))))
         wu_DI = wu_LaplaPol2PolGrid_wu(wu_DI,cond='R')
...
end program wu_diff_cn_polvel_R
