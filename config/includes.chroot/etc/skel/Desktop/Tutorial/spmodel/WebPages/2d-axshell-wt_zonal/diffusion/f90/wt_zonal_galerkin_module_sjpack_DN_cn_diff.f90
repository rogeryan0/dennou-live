...
!      外側で温度固定(T=0), 内側でフラックス固定条件(dT/dr=0)
!
...
!
!履歴  2009/10/04  竹広真一 wt_zonal_galerkin_DN_cn_diff.f90 より sjpack 用に改造
!
program wt_zonal_galerkin_module_sjpack_DN_cn_diff
  !
 ...
 !---- 変数 ----
  ...
  real(8), dimension(nm+1,2:lm)      :: wp_Temp    ! 温度
...
contains
  ...
  function det(l,x)
    !
    ! 係数行列式の計算
    ! Δ=sbj1(ri*x)sbn(ro*x)-sbn1(ri*x)sbj(ro*x)
    !
  ...
  function CalAlpha(l,n)
    !
    ! sbj1(ri*x)sbn(ro*x)-sbn1(ri*x)sbj(ro*x) の解を求める
    !
    integer, intent(IN) :: l                 ! 次数
...
99  continue
   ...
   !------------------- 拡散項 ----------------------
    subroutine CNDiffusionMatrixDN( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      real(8)               :: wp_I(nm+1,2:lm)       ! 作業用変数

      integer :: l

      DiffLUMatrix = 0.0
      do l=2,lm
         wp_I = 0.0 ; wp_I(:,l) = 1.0             ! 各波数成分独立
         DiffLUMatrix(:,:,l) &
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    ...
    call HistoryCreate( &                                  ! ヒストリー作成
           file='wt_zonal_galerkin_module_sjpack_DN_cn_diff.nc', &
...
end program wt_zonal_galerkin_module_sjpack_DN_cn_diff
