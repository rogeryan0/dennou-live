...
!
program wq_diff_cn_polvel_tau_R
  !
  !  wq_module サンプルプログラム
  !      球内のポロイダル速度場拡散問題(Crank-Nicolson scheme)
  !      外側粘着条件
 ...
 !---- 空間解像度設定 ----
  ...
  integer,parameter  :: nm=5,  lm=31         ! 切断波数の設定(水平, 動径)
 ...
 !---- 変数 ----
  ...
  real(8), dimension((nm+1)**2,0:lm) :: wq_Polvel      ! ポロイダル速度場
  real(8), dimension((nm+1)**2,km)   :: wr_Polvel      ! ポロイダル速度場
  real(8), dimension((nm+1)**2,0:lm) :: wq_LaplaPolvel ! ▽^2ポロイダル
 ...
 !------------------- 初期値設定 ----------------------
  ...
  wq_Polvel = wq_wr(wr_Polvel)
  wq_LaplaPolvel = wq_Lapla_wq(wq_Polvel)
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                               ! Crank-Nicolson 法による時間積分
     ...
     wq_LaplaPolvel = wq_LaplaPolvel &
          + dt/2 *( nu * wq_Lapla_wq(wq_LaplaPolvel)  )
     wq_Polvel = wq_LaplaPol2Pol_wq(wq_LaplaPolvel,cond='R')
...
99  continue
    ...
    subroutine CNDiffusionMatrixPolvel( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      real(8)               :: wq_I((nm+1)*(nm+1),0:lm)       ! 作業用変数
      real(8)               :: wq_DI((nm+1)*(nm+1),0:lm)       ! 作業用変数
      ...
      do l=0,lm
         ...
         wq_DI = - Diffc * dt/2.0 &
                  * wq_Lapla_wq(wq_Lapla_wq(wq_I))
         wq_DI = wq_LaplaPol2Pol_wq(wq_DI,cond='R')
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    ...
    call HistoryCreate( &                                  ! ヒストリー作成
           file='wq_diff_cn_polvel_tau_R.nc', &
...
end program wq_diff_cn_polvel_tau_R
