...
!
program chsbaro_beta_freedecay_euler_exp1
...
!== 宣言部 ============================================================
 ...
 !---- 変数(スペクトルデータ) ----
  ...
  real(8), allocatable :: es_DVorDt(:,:)      ! 渦度時間変化(t)

  real(8), allocatable :: es_ESp(:,:)         ! エネルギースペクトル
  real(8), allocatable :: es_EnsSp(:,:)       ! エンストロフィースペクトル
 ...
 !---- NAMELIST 変数 ----
  ...
  character(len=100)  :: nmlfile='chsbaro-beta_freedecay_euler_exp1.nml'
 ...
 !---------------- NAMELIST 読み込み ---------------------
  ...
  if ( size(argv) .le. 0 ) then
     call MessageNotify('W','chsbaro-beta_euler_exp1',&
                            'There is no argument. The defalut namlist file, '&
                            //trim(nmlfile)//' is used.')
  else
     nmlfile=argv(1)
     call MessageNotify('M','chsbaro-beta_euler_exp1',&
 ...
 !------------------ 変数の割り付け ---------------------
  ...
  allocate(es_DVorDt(-km:km,lm))
  allocate(es_HVisc(-km:km,lm))
  allocate(es_ESp(-km:km,lm),es_EnsSp(-km:km,lm))
...
&                 c2='Time integration starts.') 
  ...
  do it=1,nstep
     ...
     ! 時間変化項の見積り
     es_DVorDt = - es_Jacobian_es_es(es_StrFunc,es_Vor)  &
                 - Beta * es_Dx_es(es_StrFunc) &
                 - es_HVisc * es_Vor

     ! 時間積分計算(Euler scheme)
     es_Vor = es_Vor + delta_t * es_DVorDt
    ...
    do k=-km,km
       do l=-lm,lm
          ...
          if ( (Nmin .le. Ntotal) .AND. (Ntotal .le. Nmax) ) then
             ...
             es_StrFunc(l,k) = 2.0 * harvest - 1     ! [-1,1] の一様乱数
          endif
       enddo
    enddo
 ...
 !
  subroutine output_restart_init
    call HistoryCreate( &
           file=trim(rst_file), &
           title=trim(title), &
           source='plbaro-beta_freedecay_euler_exp1.f90 (2008/10/11)', &
 ...
 !
  subroutine output_history_init
    ...
    call HistoryCreate( &
           file=trim(hst_file), &
           title=trim(title), &
           source='chsbaro-beta_freedecay_euler_exp1.f90 (2008/10/11)', &
           institution='GFD_Dennou Club SPMODEL project',     &
           dims=(/'x','y','k','l','t'/), &
           dimsizes=(/im,jm+1,2*km+1,lm,0/),&
           longnames=(/'X           ','Y           ',&
...
end program chsbaro_beta_freedecay_euler_exp2
