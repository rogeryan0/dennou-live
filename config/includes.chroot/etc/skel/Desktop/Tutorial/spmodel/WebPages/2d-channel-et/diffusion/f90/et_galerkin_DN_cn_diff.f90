...
!      
!
program et_galerkin_DN_cn_diff
  !
 ...
 !---- 変数 ----
  ...
  real(8)             :: ep_Phi(-km:km,2:lm)     ! ガラーキンデータ(温度)
 ...
 !--- Crank Nicholson 陰的計算用作業変数 ---
  ...
  integer :: kpivot_DN(-km:km,2:lm) ! Crank Nicholson 用行列ピボット情報
  ...
  contains
   ...
   !------------------- 拡散項 ----------------------
    subroutine CNDiffusionMatrixDN( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      real(8)               :: ep_I(-km:km,2:lm)              ! 作業用変数

      integer :: l

      DiffLUMatrix = 0.0
      do l=2,lm
         ep_I = 0.0 ; ep_I(:,l) = 1.0             ! 各波数成分独立
         DiffLUMatrix(:,:,l) &
   ...
   !------------------- 出力 ----------------------
    subroutine output_gtool4_init
      ...
      call HistoryCreate( &                               ! ヒストリー作成
           file='et_galerkin_DN_cn_diff.nc', &
...
end program et_galerkin_DN_cn_diff
