...
!      2005/02/24  竹広 真一   ストークス近似解との比較用
!      2008/08/10  竹広 真一   配列添え字変更 im -> 0:im-1
!
program NS_rot_3d_shell_abcn_restart_namelist_rotbndry_StokeCheck
 ...
 ! 上下トロイダルポテンシャル境界値
  real(8), dimension(0:im-1,jm)        :: xy_TorvelBoundaryTop
  real(8), dimension(0:im-1,jm)        :: xy_TorvelBoundaryBottom
  real(8), dimension((nm+1)*(nm+1))    :: w_TorvelBoundaryTop
  real(8), dimension((nm+1)*(nm+1))    :: w_TorvelBoundaryBottom

 !---- その他 ----
 ! トロイダル速度(ストークス近似での解析定常解)
  real(8), dimension(0:im-1,jm,0:km)     :: xyz_TorvelStokesSol
 ...
 !---- NAMELIST 変数 ----
  ...
  real(8)            :: Re=1.0D0                 ! レイノルズ数
  real(8)            :: RsInv=0.0D0              ! 1/ロスビー数
  real(8)            :: ReInv=0.0D0              ! 1/レイノルズ数
  namelist /physics/  Rs, Re                     ! 
                                                 ! -- 境界条件 --
  character(len=2)   :: VelBC='RR'               ! 速度境界条件(RR/RF/FR/FF)
  real(8)            :: OmegaTop=1.0D0           ! 上境界回転角速度
  real(8)            :: LonOmegaTop=0.0D0        ! 上境界回転軸経度
  real(8)            :: LatOmegaTop=90.0D0       ! 上境界回転軸緯度
  real(8)            :: OmegaBottom=1.0D0        ! 下境界回転角速度
  real(8)            :: LonOmegaBottom=0.0D0     ! 下境界回転軸経度
  real(8)            :: LatOmegaBottom=90.0D0    ! 下境界回転軸緯度
  namelist /boundary/ VelBC, &
                      OmegaTop,    LonOmegaTop,    LatOmegaTop,   &
                      OmegaBottom, LonOmegaBottom, LatOmegaBottom
 ...
 !------------------ 境界条件設定 -------------------
  xy_TorvelBoundaryTop = OmegaTop * Ro &
                          * (  cos(LatOmegaTop/180.0*pi)*cos(xy_Lat)     &
                                 * cos(LonOmegaTop/180.0D0*pi-xy_Lon)    &
                              + sin(LatOmegaTop/180.0D0*pi)*sin(xy_Lat) )
  w_TorvelBoundaryTop=w_xy(xy_TorvelBoundaryTop)

  xy_TorvelBoundaryBottom = OmegaBottom * Ri &
                          * (  cos(LatOmegaBottom/180.0*pi)*cos(xy_Lat)   &
                                 * cos(LonOmegaBottom/180.0D0*pi-xy_Lon)  &
                              + sin(LatOmegaBottom/180.0D0*pi)*sin(xy_Lat) )
  w_TorvelBoundaryBottom=w_xy(xy_TorvelBoundaryBottom)
 ...
 !------------------- 時間積分(Adams-Bashforth 法) --------------------
  ...
  do it=1,nstep
     ...
     call wt_TorBoundariesGrid(wt_Torvel,cond=velBC, &
          values=(/w_TorvelBoundaryTop,w_TorvelBoundaryBottom/))
...
! 以下 サブルーチン
 ...
 !------------------- 出力 ----------------------
  subroutine output_gtool4_init
   ...
   !---- 解析解定義・出力 ----
    call HistoryAddVariable( &                                ! 変数定義
           varname='torvel_stokessol', dims=(/'lon','lat','rad'/), & 
           longname='toroidal velocity potential (Analytical steady solution under stokes approx.', units='1', xtype='double')

    xyz_TorvelStokesSol = &
         -ri**3/(Ro**3-ri**3)*(1-ro**3/xyz_Rad**3 ) * OmegaBottom * xyz_Rad &
             *( cos(xyz_Lat)*cos(LatOmegaBottom*pi/180.0D0)  &
                     *cos(xyz_Lon-LonOmegaBottom*pi/180.0D0) &
               + sin(xyz_Lat)*sin(LatOmegaBottom*pi/180.0D0) ) &
         +ro**3/(ro**3-ri**3)*(1-ri**3/xyz_Rad**3 ) * OmegaTop * xyz_Rad &
            *( cos(xyz_Lat)*cos(LatOmegaTop*pi/180.0D0)   &
                     *cos(xyz_Lon-LonOmegaTop*pi/180.0D0) &
             + sin(xyz_Lat)*sin(LatOmegaTop*pi/180.0D0) )

    call HistoryPut('torvel_stokessol',xyz_TorvelStokesSol)    ! 変数出力
...
end program NS_rot_3d_shell_abcn_restart_namelist_rotbndry_StokeCheck
