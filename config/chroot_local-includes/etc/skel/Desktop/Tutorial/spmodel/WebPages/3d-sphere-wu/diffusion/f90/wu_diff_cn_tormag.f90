...
program wu_diff_cn_tormag
 ...
 !---- 変数 ----
  real(8), dimension(0:im-1,jm,0:km) :: xyr_Tormag   ! トロイダル磁場
  real(8), dimension((nm+1)**2,0:lm) :: wu_Tormag    ! トロイダル磁場
  real(8), dimension((nm+1)**2,0:km) :: wr_Tormag    ! トロイダル磁場

  real(8), dimension(0:im-1,jm,0:km) :: xyr_TormagInit ! トロイダル磁場(初期値)
  real(8), dimension(0:im-1,jm,0:km) :: xyr_TormagSol  ! トロイダル磁場(解析解)
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                               ! Crank-Nicolson 法による時間積分
     ...
     wu_Tormag = LUSolve(DifLUMT_Tormag,kpivot_Tormag,wu_Tormag)
