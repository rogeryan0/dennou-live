...
{ {\large SPMODEL サンプルプログラム} \\  % ここは変更しない 
   水路領域赤道 $\beta$ 面浅水モデル: \\
   2 つの局所熱源に対する線形応答問題
  {\large shallow\_eqbeta2.f90}                   % プログラム名
}
%
...
SPMODEL サンプルプログラム『shallow\_eqbeta2.f90』に用いられている基礎方
...
として, 赤道上の 2 つの局在した質量源を与えた場合の応答を計算する.


{\bf プログラム名} \\                  % プログラム名
{\footnotesize
shallow\_eqbeta2.f90
}

{\bf プログラム取得元}\\               % プログラム取得先
{\footnotesize
http:\slash \slash www.gfd-dennou.org\slash arch\slash spmodel\slash 2d-channel-esc\slash shallow-equator\slash sample2\slash SIGEN.htm
}
...
数値実験として, 赤道上に 2 つの局在した符号の異なる質量源を与えた場合の
応答を計算する.質量源分布は以下のように与える.
\begin{eqnarray}
Q &=& Q_{1} + Q_{2}, \\
Q_{1} &=& \left\{
\begin{array}{lc}
Q_{1_0}\exp (y^{2}/b_{1}) \cos \left(\frac{\pi (x-x_{1})}{2a_{1}}\right) 
& - a_{1} \le x \le a_{1} \\ 
0 & x < -a_{1}, x > a_{1}
\end{array}
\right. ,\\
Q_{2} &=& \left\{
\begin{array}{lc}
Q_{2_0}\exp (y^{2}/b_{2}) \cos \left(\frac{\pi (x-x_{2})}{2a_{2}}\right) 
& - a_{2} \le x \le a_{2} \\ 
0 & x < -a_{2}, x > a_{2}
\end{array}
\right. .
...
時間格子間隔 $\Delta t$ は $0.2$, 計算ステップ数は 3,000 ステップである.
...
\Dfigref{shallow-eqbeta2.f90 の結果}に厚さ分布の時間変化の様子を示す.
2 つの質量源の東側では負のケルビン応答と正のケルビン応答が打ち消しあった
結果として, 負のケルビン応答の波頭が伝播する様子のみが見られる. 十分時間
が経過した後には循環は 2 つの質量源の間のみに残る\footnote{ 
\Dfigref{shallow-eqbeta2.f90 の結果} (流体厚さ) では明らかに見ることはで
きない}. 


\begin{figure}[p]
\begin{center}
\Depsf[15cm][]{./figs/shallow_eqbeta2.ps}
\vspace*{-1.0cm}
\caption{厚さの分布の時間変化の様子. 右上から左下へに 
         $t=40$ から 40 毎の結果.}
\Dfiglab{shallow-eqbeta2.f90 の結果}
\end{center}
\end{figure}
