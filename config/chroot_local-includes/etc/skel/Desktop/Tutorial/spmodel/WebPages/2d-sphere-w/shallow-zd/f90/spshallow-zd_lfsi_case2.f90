...
!== 宣言部 ============================================================
 ...
 !---- NAMELIST 変数 ----
  ...
  character(len=30),parameter  :: nmlfile='spshallow-zd_lfsi_case2.nml'
...
&                 c2='Time integration starts.') 
  ...
  do it=1,nstep
     ...
     w_HsfcA = (1-delta_t*w_HDiff) * w_HsfcB + delta_t * w_DHsfcDtNoGrav

     w_DivA  = ( (1-delta_t*w_HVisc)*w_DivB + w_DDivDtNoGrav * delta_t &
                   - delta_t*Grav*w_Lapla_w(w_HsfcA)/Radius**2 )        &
               /(1+delta_t**2*Grav*Hbar*(-rn(:,1))/Radius**2)

     w_HsfcA = w_HsfcA - delta_t * Hbar * w_DivA 

     w_DivA  = 2 * w_DivA  - w_DivB
     w_HsfcA = 2 * w_HsfcA - w_HsfcB
 ...
 !
  subroutine output_restart_init
    call HistoryCreate( &
           file=trim(rst_file), &
           title=trim(title), &
           source='spshallow-zd_lfsi_case2.f90 (2008/08/10)', &
...
end program spshallow_zd_lfsi_case2
