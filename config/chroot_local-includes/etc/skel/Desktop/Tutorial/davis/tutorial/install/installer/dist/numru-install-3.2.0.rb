...
$Id: numru-install.rb,v 1.17 2004/08/12 08:49:48 koshiro Exp $

Change Log:

* 2004/08/12 KOSHIRO Tsuyoshi <koshiro@gfd-dennou.org>
  - (3.2.0)
  - change class RbLib: deal with changing installation script of narray_miss.

* 2004/08/11 KOSHIRO Tsuyoshi <koshiro@gfd-dennou.org>
  - (3.1.0)
  - change class RubySoft: deal with changing installation script of gave.
  - name of package is changed: misc -> numru-misc
  - bug fix.

* 2004/07/15 KOSHIRO Tsuyoshi <koshiro@gfd-dennou.org>
  - (3.0.0)
...
* 2003/12/18 KOSHIRO Tsuyoshi <koshiro@gfd-dennou.org>
  - (2.0.1)
...
* 2003/12/10 KOSHIRO Tsuyoshi <koshiro@gfd-dennou.org>
  - (2.0.0)
...
(C) 2003-2004 GFD Dennou Club      http://www.gfd-dennou.org/
...
module NumRu
		...
		def http_get_list(uri, header, proxy, list)
			...
			dir_html.split(/\n/).each do |line|
				line.scan(/<A HREF="(.*)">(.*)<\/A>/i) do |href, fname|
					list << fname if href.split(/\//)[-1] == fname
				end
			end
		end
	...
	module Install
		...
		if /^2/ =~ `pkg-config gtk+-2.0 --modversion 2>&1`
			GtkMajorVer = '2'
		elsif /^1/ =~ `gtk-config --version 2>&1` and /^1/ =~ `imlib-config --version 2>&1`
			GtkMajorVer = '1'
		else
			GtkMajorVer = '0'
		end

		Patch = (RUBY_PLATFORM !~ /cygwin/ and `type 'gpatch' > /dev/null 2>&1`) ? 'gpatch' : 'patch'
		...
		class Software
			...
			def check_latest_ver
				list = Array::new
				print '    ', @check_url.to_s, " ... \n"
				NumRu::Fetch::get_list(URI::parse(@check_url), Header, Proxy, list)
				if name == 'dcl-5.2-C'
					list.delete_if do |f|
			...
			def initialize(name, realname, comm, group, check_cmd, current_ver_method, url, depend={}, patch=[])
				...
				@depend = depend
				@patch = patch
				(@latest_fname, @latest_ver, @src_dir) = check_latest_ver
			end
			...
			def current_ver
				if self.installed?
					...
					if @name == 'dcl-5.2-C'
						...
						if /^Usage/ =~ gtk_ver or gtk_ver == 'no'
							gtk_ver = '_gtk0' 
						else
			...
			def check_depend
				...
				check_pkgs.each do |dep|
					...
					if req_ver == ''
						...
						elsif current_ver == NotInstalled or current_ver < req_ver
							dep_ver_low << dep
							dep_failed  << dep
							dep_not_cleared << dep
			...
			def install(dl_dir = DefaultDownloadDir, prefix = DefaultPrefix)
				...
				if @name == 'fftw'
					raise error_msg if not system('make clean')
		...
		class RubySoft < Software
			...
			def install(dl_dir = DefaultDownloadDir, rbsitelibdir = DefaultRbSiteLibDir, prefix = DefaultPrefix)
				...
				case @name
				when 'gpv'
					raise error_msg if not system('mv -f install.rb install.rb.orig')
					esc_rbsitelibdir = rbsitelibdir.gsub(/\//, '\/')
					raise error_msg if not system("sed -e 's/libdir = prefix.*$/libdir = \"#{esc_rbsitelibdir}\\/numru\"/' install.rb.orig > install.rb")
					raise error_msg if not system("ruby install.rb #{prefix}")
				when 'gave'
					raise error_msg if not system("ruby setup.rb config --prefix=#{prefix} --site-ruby=#{rbsitelibdir}")
					raise error_msg if not system("ruby setup.rb install")
				end
				Dir::chdir(cur_dir)
				add_path(prefix+'/bin')
		...
		class RbExtLib < RubyLib
			def install(dl_dir = DefaultDownloadDir, rbsitelibdir = DefaultRbSiteLibDir, prefix = DefaultPrefix)
				...
				if @name == 'ruby-dcl' or @name == 'ruby-netcdf' or @name == 'ruby-fftw3'
					conf_opt = Array::new
				...
				Dir::glob("./**/Makefile").each do |mkfile|
					raise error_msg if not system("mv -f #{mkfile} #{mkfile}.orig")
					esc_rbsitelibdir = rbsitelibdir.gsub(/\//, '\/')
					raise error_msg if not system("sed -e 's/\\$(sitearchdir)\\(\\$(target_prefix)\\)/#{esc_rbsitelibdir}\\/\\$(arch)\\1/' -e 's/\\$(sitelibdir)\\(\\$(target_prefix)\\)/#{esc_rbsitelibdir}\\1/' #{mkfile}.orig > #{mkfile}")
				end
				if not system('make')
					raise error_msg if not system('make')
				end
				raise error_msg if not system('make site-install')
				if @name == 'ruby-gnome2-all' and /cygwin/ =~ RUBY_PLATFORM
					Dir::glob("./**/lib*.a").delete_if{|implib| /src2./ =~ implib}.each do |implib|
						raise error_msg if not File::install(implib, "#{rbsitelibdir}/#{Config::CONFIG['arch']}", 0644, true)
					end
				end
				Dir::chdir(cur_dir)
		...
		class RbLib < RubyLib
			def install(dl_dir = DefaultDownloadDir, rbsitelibdir = DefaultRbSiteLibDir, prefix = DefaultPrefix)
				...
				if @name == 'narray_miss'
					raise error_msg if not system("ruby setup.rb config --site-ruby=#{rbsitelibdir}")
					raise error_msg if not system("ruby setup.rb install")
				else
				...
				if @name == 'gphys'
					Dir::chdir("bin")
					raise error_msg if not system('mv -f install.rb install.rb.orig')
					esc_prefix = prefix.gsub(/\//, '\/')
					raise error_msg if not system("sed -e 's/bindir = .*$/bindir = \"#{esc_prefix}\\/bin\"/' install.rb.orig > install.rb")
					raise error_msg if not system('/bin/rm -f install.rb.orig')
					raise error_msg if not system("ruby install.rb")
				end
				Dir::chdir(cur_dir)
				self.save_latest_ver if @current_ver_method != 'ModuleConst'
				add_rb_load_path(rbsitelibdir)
			...
			software.each do |soft|
				if soft.installed?
					...
					if Regexp.new(Regexp.quote(Software::VersionUnknown)) =~ current_ver
						unknown << soft
						not_cleared << soft 
						if /gtk[0-2]$/ =~ current_ver and GtkMajorVer > current_ver[-1].chr
							gtk_low << soft
							failed << soft
						end
						...
						failed << soft
						not_cleared << soft 
					elsif /gtk[0-2]$/ =~ current_ver and GtkMajorVer > current_ver[-1].chr
						gtk_low << soft
						failed << soft
						not_cleared << soft
					else
...
if __FILE__ == $0
	...
	dcl_c = Software.new(
		...
		'http://www.gfd-dennou.org/arch/ruby/products/ruby-dcl/dcl-C/',
		{},
		['http://www.gfd-dennou.org/arch/ruby/products/installer/patches/dcl52c_cygwin.patch.gz']
	)
	netcdf = Software.new(
		'netcdf', 'NetCDF',
		...
		{},
		['http://www.gfd-dennou.org/arch/ruby/products/installer/patches/netcdf350_cygwin.patch.gz']
	)
	fftw = Software.new(
		'fftw', 'FFTW',
	...
	if GtkMajorVer == '1'
		...
		rubygnome = RbExtLib.new(
			...
			'http://prdownloads.sourceforge.net/ruby-gnome/ | http://voxel.dl.sourceforge.net/sourceforge/ruby-gnome/',
			{}
		)
	else
		rubygnome2 = RbExtLib.new(
			...
			'gtk2', 'ModuleConst',
			'http://prdownloads.sourceforge.net/ruby-gnome2/ | http://voxel.dl.sourceforge.net/sourceforge/ruby-gnome2/',
			{}
		)
	end
	...
	while not stop_loop
		...
		num_soft = -1
		first = true
		until num_soft >= 0 and num_soft <= software.size
			print "\n    ----> Invalid number.\n" if not first
		...
		if num_soft == 0
			...
			if select_soft.name == 'dcl-5.2-C' and RUBY_PLATFORM =~ /cygwin/
			...
			if /gtk[0-2]$/ =~ rubydcl.current_ver
				if select_soft.name == 'dcl-5.2-C' and GtkMajorVer > rubydcl.current_ver[-1].chr
      ...
      ask_prefix = false
		inst_soft.each do |soft|
			if soft.group == 'Library' or  soft.group == 'RubySoft' or soft.name == 'gphys'
				ask_prefix = true 
				break
			end
		end
      ...
      ask_rbsitelibdir = false
		inst_soft.each do |soft|
			if ( soft.group == 'RubySoft' or  /^Rb/ =~ soft.group )
