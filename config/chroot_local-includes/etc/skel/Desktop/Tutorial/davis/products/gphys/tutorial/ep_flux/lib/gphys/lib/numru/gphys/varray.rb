...
=end
   ...
   class VArray
	...
	if self.class == VArray
	  ...
	  # subclass --> VArray
	  VArray.new(narray, attr.copy, name)
	end
      end
	...
	if myunits != to
	 ...
	 if @mapping
      ...
      def transpose(*dims)
	VArray.new( val.transpose(*dims), @attr.copy, name )
      end
	...
	when UNumeric
	  ...
	  oattr['units'] = '1'    # Assume the other to be non-dimensional
	  case other
	  when Numeric
	...
	[c_other, self]
      ...
      Binary_operators_Uconv = ["+","-",".add!",".sub!"]
	 ...
	 def #{f}(*arg)
      ...
      def sqrt
	va = VArray.new( Misc::EMath.sqrt(self.val), self.attr.copy, self.name )
	...
	va
	 ...
	 def #{f.delete(".")}(other)
	    case other
            when VArray, UNumeric
               ary = self.val#{f}(other.val)
	       va = VArray.new( ary, self.attr.copy, self.name )
	       va.units= self.units#{f}(other.units)
               va
	    when Numeric, NArray, NArrayMiss, Array
	       ary = self.val#{f}(other)
	       va = VArray.new( ary, self.attr.copy, self.name )
	       if "#{f}" == "**" 
	 ...
	 def #{f.delete(".")}(other)
	    case other
            when VArray, UNumeric
               ary = self.val#{f}( other.units.convert2(other.val,units) )
