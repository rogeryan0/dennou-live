...
!----------------------------------------------------------------------
!表題  wa_zonal_module_sjpack
!
!   spml/wa_zonal_module_sjpack モジュールは球面上での経度方向に一様な
!   帯状的 1 次元流体運動をルジャンドル多項式を用いたスペクトル法によって
!   数値計算するための Fortran90 関数を提供する. 
!
!   球面上の 1 層モデル用 w_zonal_module_sjpack モジュールを多層モデル用に
!   拡張したものであり, 同時に複数個のスペクトルデータ, 格子点データに
!   対する変換が行える.
!
!   内部で l_module_sjpack/la_module_sjpack を用いている. 最下部では
!   ルジャンドル多項式変換のエンジンとして ISPACK の Fortran77 サブルーチンを
!   用いている.
!
!   関数, サブルーチンの名前と機能は wa_zonal_module のものと同じに
!   設計してある. したがって use 文を wa_zonal_module から 
!   wa_zonal_module_sjpack に変更するだけで SJPACK の機能が使えるようになる. 
!
!
!履歴  2009/09/22  竹広真一  wa_zonal_module をもとに SJPACK 用に改造
!
!++
module wa_zonal_module_sjpack
  !
  != wa_zonal_module_sjpack
  !
  ! Authors:: Shin-ichi Takehiro, Youhei SASAKI
  ! Version:: $Id: wa_zonal_module_sjpack.f90,v 1.2 2009-09-25 12:05:12 takepiro Exp $
  ...
  !   用いている.
  !
  !   関数, サブルーチンの名前と機能は wa_zonal_module のものと同じに設計してある. 
  !   したがって use 文を l_module から wa_zonal_module_sjpack に変更するだけで 
  !   SJPACK の機能が使えるようになる. 
  !
  !== 関数・変数の名前と型について
  !
  ! w_zonal_module_sjpack でのものに追加される関数・変数の名前と型についてのみ
  ! 説明する. w_zonal_module_sjpack での関数・変数の名前と型については 
  ! w_zonal_module_sjpack の該当項を参照されたい.
  !
  !=== 命名法
  !
  ...
  use la_module_sjpack, wa_ya => la_ya, ya_wa => ya_la, &
  ...
  !--------------- 初期化 -----------------
    subroutine wa_Initial(n_in,i_in,j_in,k_in,np_in)
      ...
      call MessageNotify('M','wa_initial',&
           'wa_zonal_module_sjpack (2009/09/22) is initialized')
...
end module wa_zonal_module_sjpack
