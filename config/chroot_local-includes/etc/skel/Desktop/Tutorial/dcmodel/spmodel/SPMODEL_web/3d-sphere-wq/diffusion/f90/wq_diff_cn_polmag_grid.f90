...
!
program wq_diff_cn_polmag_grid
 ...
 ! Crank Nicholson 陰的計算用拡散行列(ポロイダル磁場)
  real(8)             :: DifLUMT_Polmag((nm+1)*(nm+1),km,km)
  integer             :: kpivot_Polmag((nm+1)*(nm+1),km)    ! ピボット
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                               ! Crank-Nicolson 法による時間積分
     ...
     wr_Polmag = LUSolve(DifLUMT_Polmag,kpivot_Polmag,wr_Polmag)
 ...
 !------------------- 拡散項 ----------------------
  subroutine CNDiffusionMatrix( Diffc, dt, DiffLUMatrix, kpivot )
    ...
    do k=1,km
       ...
       call wr_PolmagBoundaryGrid(wr_DI)
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    call HistoryCreate( &                                  ! ヒストリー作成
           file='wq_diff_cn_polmag_grid.nc', &
...
end program wq_diff_cn_polmag_grid
