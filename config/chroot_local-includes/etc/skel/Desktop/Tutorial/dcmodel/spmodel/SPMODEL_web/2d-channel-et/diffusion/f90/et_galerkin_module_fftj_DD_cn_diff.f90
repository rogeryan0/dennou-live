...
!     φ = 0 at y=-d/2,d/2
!
!履歴  2009/09/16  竹広真一  et_galerkin_DD_cn_diff.f90 を fftj 用に改造
!
program et_galerkin_module_fftj_DD_cn_diff
  !
 ...
 !---- 変数 ----
  ...
  real(8)             :: ed_Phi(-km:km,2:lm)     ! ガラーキンデータ(温度)
 ...
 !--- Crank Nicholson 陰的計算用作業変数 ---
  ...
  integer :: kpivot_DD(-km:km,2:lm) ! Crank Nicholson 用行列ピボット情報
  ...
  contains
   ...
   !------------------- 拡散項 ----------------------
    subroutine CNDiffusionMatrixDD( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      real(8)               :: ed_I(-km:km,2:lm)              ! 作業用変数

      integer :: l

      DiffLUMatrix = 0.0
      do l=2,lm
         ed_I = 0.0 ; ed_I(:,l) = 1.0             ! 各波数成分独立
         DiffLUMatrix(:,:,l) &
   ...
   !------------------- 出力 ----------------------
    subroutine output_gtool4_init
      ...
      call HistoryCreate( &                               ! ヒストリー作成
           file='et_galerkin_module_fftj_DD_cn_diff.nc', &
...
end program et_galerkin_module_fftj_DD_cn_diff
