...
! and Clank-Nicholson scheme (diffusion) with an operator splitting method.
!
program advdiff1
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt
     ...
     ! 移流計算(Adams-Bashforth)
     e_Zeta = e_Zeta - dt * (   3/2.0 * U * e_Dx_e(e_Zeta)        &
                              - 1/2.0 * U * e_Dx_e(e_Zeta_old) )

     ! 拡散計算(Crank Nicholson)
     e_Zeta = ( e_Zeta + dt * 1/2.0 * D * e_Dx_e(e_Dx_e(e_Zeta)) ) &
              / ( 1 - dt/2.0 * D * e_Lapla )
