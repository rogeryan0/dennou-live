...
!
! Solving 2-D Boussinesq fluid system (FF problem)
...
program bsnconv2
 ...
 !---- 変数 ----
  ...
  real(8)             :: et_Temp(-km:km,0:lm)   ! スペクトルデータ(温度擾乱)
 ...
 !---- 時間積分パラメター ----
  ...
  integer, parameter :: nt=100000, ndisp=5000 ! 時間積分数, 表示ステップ

 !---- 物理パラメター ----
  character(len=2), parameter :: TempBC='NN'    ! 温度境界条件(DD/DN/ND/NN)
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                                         ! Euler 法による時間積分
     ...
     call et_Boundaries(et_Zeta,e_ZetaBndry,cond='DD')
     et_Psi = et_LaplaInv_et(et_Zeta)
...
contains
    ...
    subroutine output_gtool4_init
      call HistoryCreate( &                               ! ヒストリー作成
           file='bsncnv_2_et.nc', title='Boussinesq convection (FF)', &
