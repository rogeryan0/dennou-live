...
!
!履歴  2009/10/04  竹広真一 wt_zonal_galerkin_MMex_cn_diff.f90 より sjpack 用に改造
!
program wt_zonal_galerkin_module_sjpack_MMex_cn_diff
  !
 ...
 !---- 変数 ----
  ...
  real(8), dimension(nm+1,2:lm)      :: wh_Polmag    ! ポロイダル磁場
  real(8), dimension(nm+1,0:km)      :: wz_Polmag    ! ポロイダル磁場

  real(8), dimension(0:im-1,jm,0:km) :: xyz_PolmagInit ! ポロイダル磁場(初期値)
  real(8), dimension(0:im-1,jm,0:km) :: xyz_PolmagSol  ! ポロイダル磁場(解析解)

 ! Crank Nicholson 設定
  real(8)             :: DifLUMT_Polmag(nm+1,0:lm,0:lm)
                         ! Crank Nicholson 陰的計算用拡散行列(ポロイダル磁場)
  integer             :: kpivot_Polmag(nm+1,0:lm)   
                         ! Crank Nicholson 陰的計算用拡散行列ピボット
 ! 境界条件設定用配列
  real(8)      :: cfdx0_ri(nm+1)    ! 境界条件設定用配列(0階微分@r=ri)
  real(8)      :: cfdx1_ri(nm+1)    ! 境界条件設定用配列(1階微分@r=ri)
  real(8)      :: cfdx0_ro(nm+1)    ! 境界条件設定用配列(0階微分@r=ro)
  real(8)      :: cfdx1_ro(nm+1)    ! 境界条件設定用配列(1階微分@r=ro)
 ...
 !---------------- 座標値の設定 ---------------------
  do n=0,nm
     cfdx1_ri(l_nm(n,0)) = 1.0D0
     cfdx0_ri(l_nm(n,0)) = -n/ri
     cfdx1_ro(l_nm(n,0)) = 1.0D0
     cfdx0_ro(l_nm(n,0)) = (n+1)/ro
  enddo
 ...
 !------------------- 初期値設定 ----------------------
  ...
  wz_Polmag = 0.0
  do k=0,km
     wz_Polmag(l_nm(n,0),k) = -sbn(n+1,alpha*ri)*sbj(n,alpha*z_Rad(k)) &
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    ...
    call HistoryCreate( &                                  ! ヒストリー作成
           file='wt_zonal_galerkin_module_sjpack_MMex_cn_diff.nc', &
  ...
  subroutine output_gtool4
    ...
    call HistoryPut('polmag',xyz_Polmag(0,:,:))
    call HistoryPut('polmagsol',xyz_PolmagSol(0,:,:))
    call HistoryPut('polmagerror',xyz_Polmag(0,:,:)-xyz_PolmagSol(0,:,:))
...
end program wt_zonal_galerkin_module_sjpack_MMex_cn_diff
