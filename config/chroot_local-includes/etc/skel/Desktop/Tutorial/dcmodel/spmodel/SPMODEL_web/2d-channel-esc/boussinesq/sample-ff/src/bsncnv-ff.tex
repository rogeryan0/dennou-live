...
\Dtitle[{\small 2 次元ブシネスクモデル:熱フラックス固定対流}] % ヘッダ部タイトル, モデル名と同じ
{ {\large SPMODEL サンプルプログラム} \\  % ここは変更しない 
  水路領域 \\ 2 次元ブシネスク方程式モデル:\\           % モデル名
  熱フラックス固定条件下の対流問題 \\
  {\large bsncnv-ff-1.f90}             % プログラム名
}
%
...
SPMODEL サンプルプログラム『bsncnv-ff-1.f90』に用いられている基礎方程式
と境界条件, および, このプログラムを用いた数値実験の方法について解説する. 
基礎方程式は 2 次元のブシネスク方程式系である. 計算はスペクトル法を用い
て行い, 展開関数は水平方向にはフーリエ級数, 鉛直方法には境界条件にあわせ
てフーリエ正弦級数またはフーリエ余弦級数を用いる.  波数切断は三角切断で
ある. スペクトル変換と逆変換および微分演算には, SPMODEL ライブラリ(spml) 
の esc\_module を用いている. 数値実験では Ishiwatari {\it et al}. (1994) 
の Case FF を想定し, 上下境界で熱フラックスを固定した場合の対流の計算を
行う.
...
{\footnotesize
http:\slash \slash www.gfd-dennou.org\slash arch\slash spmodel\slash 2d-channel-esc\slash boussinesq\slash bsncnv-ff\slash SIGEN.htm
}
...
&&  + \mbox{Ra} \mbox{Pr}
      \frac{1}{x_{m}}\frac{1}{J} \sum _{i=0}^{I-1}\sum _{j=0}^{J-1}
        \exp\left(-\frac{2\pi i k x_{i}}{x_{m}}\right)
        \sin\left(\frac{\pi l y_{j}}{y_{m}}\right)
...
時間格子間隔 $\Delta t$ は $1.0\time 10^{-3}$, 計算ステップ数は 10,000 
ステップである.
...
\Dfigref{bsncnv-ff-1.f90 の結果}に計算された対流の様子を示す. 時間の経過
とともに対流セルは横長となり, 最終的には 2 つの循環セルを持つ流れとなる
(Ishiwatari {\it et al}., 1994).

\begin{figure}[p]
\begin{center}
\Depsf[15cm][]{./figs/bsncnv-ff-1.ps}
\vspace*{-1.0cm}
\caption{対流場の時間変化の様子. (左) 温度. (右) 流線関数
         上から順に $t=1.0, 3.0, 5.0, 7.0, 9.0$ の結果を示す.}
         \Dfiglab{bsncnv-ff-1.f90 の結果}
\end{center}
\end{figure}
