...
program wq_diff_cn_torvel_tau
 ...
 !---- 変数 ----
  ...
  real(8), dimension((nm+1)**2,0:lm) :: wq_Torvel    ! トロイダル速度
 ...
 !---- 物理パラメター ----
  ...
  character(len=1), parameter :: BCond='F'      ! 境界条件
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                               ! Crank-Nicolson 法による時間積分
     time = it * dt
     wq_Torvel = wq_Torvel + dt/2 * nu * wq_Lapla_wq(wq_Torvel)
     call wq_TorBoundary(wq_Torvel,cond=Bcond)
     wq_Torvel = LUSolve(DifLUMT_Torvel,kpivot_Torvel,wq_Torvel)
...
99    continue
 ...
 !------------------- 拡散項 ----------------------
  subroutine CNDiffusionMatrix( Diffc, dt, DiffLUMatrix, kpivot )
    ...
    do l=0,lm
       ...
       call wq_TorBoundary(wq_DI,cond=Bcond)
 ...
 !------------------- 出力ルーチン ----------------------
  ...
  subroutine output_gtool4
    write(6,*) 'it = ',it
    if ( Bcond == 'F' ) then
       xyr_Torvel = xyr_wq(wq_RadDRad_wq(wq_TorVel)-wq_TorVel)/xyr_Rad 
    else
...
end program wq_diff_cn_torvel_tau
