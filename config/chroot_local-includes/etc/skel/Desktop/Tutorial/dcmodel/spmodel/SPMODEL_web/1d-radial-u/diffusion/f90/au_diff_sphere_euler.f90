...
program au_diff_sphere_euler
 ...
 !---- 物理パラメター ----
  ...
  integer, parameter :: norder=2                ! 固有関数の次数
 ...
 !------------------- 初期値設定 ----------------------
!  g_temp = 1.0
  g_temp = sin(norder*pi/ra*g_R)/(norder*pi/ra*g_R)  ! 固有モード
  u_temp = u_g(g_temp)
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt
     time = it * dt
     u_temp = u_temp &
          + dt * kappa * u_g(1.0D0/g_R** 2* g_Dr_u(u_g(g_R**2*g_Dr_u(u_temp))))
...
end program au_diff_sphere_euler
