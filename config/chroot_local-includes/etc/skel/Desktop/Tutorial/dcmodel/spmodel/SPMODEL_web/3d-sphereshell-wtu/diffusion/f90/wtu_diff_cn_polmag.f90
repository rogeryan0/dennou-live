...
program wtu_diff_cn_polmag
 ...
 !---- 変数 ----
  ...
  real(8), dimension(0:im-1,jm,0:kmi) :: xyr_Polmag   ! トロイダル磁場
  real(8), dimension((nm+1)**2,0:lmi) :: wu_Polmag    ! トロイダル磁場
  real(8), dimension((nm+1)**2,0:kmi) :: wr_Polmag    ! トロイダル磁場

  real(8), dimension(0:im-1,jm,0:kmi) :: xyr_PolmagInit ! トロイダル磁場(初期値)
  real(8), dimension(0:im-1,jm,0:kmi) :: xyr_PolmagSol  ! トロイダル磁場(解析解)
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                               ! Crank-Nicolson 法による時間積分
     ...
     call CNImplicitPolmag(wt_Polmag,wu_Polmag,kappa,kappa,dt)
...
99    continue
 ...
 !------------------- 拡散項 ----------------------
  subroutine CNImplicitPolmag(wt_Polmag,wu_Polmag,Diffc_o,Diffc_i,dt)

    ! トロイダル磁場(球殻)
    real(8), dimension((nm+1)*(nm+1),0:lmo), intent(INOUT) :: wt_Polmag
    ! トロイダル磁場(球)
    real(8), dimension((nm+1)*(nm+1),0:lmi), intent(INOUT) :: wu_Polmag
    ...
    real(8), dimension((nm+1)**2,0:lmo+lmi+1)   :: wtu_Polmag  ! トロイダル磁場
    ...
    if ( first ) then
       ...
       do l=0,lmo
          ...
          call wtu_PolmagBoundariesGrid(wt_DI,wu_DI)
    ...
    wtu_Polmag(:,0:lmo) = wt_Polmag
    wtu_Polmag(:,lmo+1:lmo+lmi+1) = wu_Polmag
    wtu_Polmag = LUSolve(DiffLUMatrix,kpivot,wtu_Polmag)
    wt_Polmag = wtu_Polmag(:,0:lmo) 
    wu_Polmag = wtu_Polmag(:,lmo+1:lmo+lmi+1)
...
end program wtu_diff_cn_polmag
