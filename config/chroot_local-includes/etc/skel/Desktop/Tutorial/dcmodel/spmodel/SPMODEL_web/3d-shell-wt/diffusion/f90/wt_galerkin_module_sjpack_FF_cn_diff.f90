...
!
!履歴  2009/09/18  竹広真一  wt_galerkin_SS_cn_diff.f90 を sjpack 用に改造
!                            境界条件スイッチ名を S -> F に変更
!
program wt_galerkin_module_sjpack_FF_cn_diff
  !
 ...
 !---- 変数 ----
  ...
  real(8), dimension((nm+1)**2,4:lm) :: wq_Polvel      ! ポロイダル速度場
 ...
 !---------------- 座標値の設定 ---------------------
  call wt_galerkin_Initial(im,jm,km,nm,lm,ri,ro,velBC='FF')
...
contains
  ...
  function sbn2(n,x)
    ...
    sbn2 = -2*sbn1(n,x)/x - (1-n*(n+1)/x**2)*sbn(n,x)
  end function sbn2
  ...
  function det(l,x)
    ...
    a(3,1) = l*(l-1)*ri**(l-2)  ; a(3,2) = (l+1)*(l+2)*ri**(-(l+3))
    a(3,3) = x**2*sbj2(l,ri*x)  ; a(3,4) = x**2*sbn2(l,ri*x)

    a(4,1) = l*(l-1)*ro**(l-2)  ; a(4,2) = (l+1)*(l+2)*ro**(-(l+3))
    a(4,3) = x**2*sbj2(l,ro*x)  ; a(4,4) = x**2*sbn2(l,ro*x)
  ...
  function CalcCoeff(l,alpha)
    ...
    a(3,1) = l*(l-1)*ri**(l-2)         ; a(3,2) = (l+1)*(l+2)*ri**(-(l+3))
    a(3,3) = alpha**2*sbj2(l,ri*alpha) ; b(3) = alpha**2*sbn2(l,ri*alpha)
...
99  continue
    ...
    subroutine CNDiffusionMatrixPolvel( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      ! Crank Nicholson 設定
      real(8), intent(OUT)  :: DiffLUMatrix((nm+1)*(nm+1),4:lm,4:lm)
                              ! Crank Nicholson 拡散陰的計算用行列
                              ! (1-D dt/2▽^2, LU 分解)

      integer, intent(OUT)  :: kpivot((nm+1)*(nm+1),4:lm)     ! ピボット情報
                              ! Crank Nicholson 拡散陰的計算用行列ピボット情報

      real(8)               :: wq_I((nm+1)*(nm+1),4:lm)       ! 作業用変数
      real(8)               :: wt_I((nm+1)*(nm+1),0:lm)       ! 作業用変数
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    ...
    call HistoryCreate( &                                  ! ヒストリー作成
           file='wt_galerkin_module_sjpack_FF_cn_diff.nc', &
...
end program wt_galerkin_module_sjpack_FF_cn_diff
