#!/usr/bin/env bash
#
# NS_rot-3d-shell_abcn_restart_namelist_rotbndry_StokesCheck.f90 用実行シェルスクリプト
#
...
TITLE="Viscid fluid motion in a spherical shell with rotating boundaries and steady analytical solution under Stokes approx. (T10N8)"
...
#######################################################################
 ...
 &tint    delta_t= 5.0d-005,  nstep= 1000       /

 &output  output_file='$OUTPUTFILE', NDISP= 100, title='$TITLE' /

 &physics Rs=0.0D0, Re=1.0D-1 /
