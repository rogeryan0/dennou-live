...
!      
!
program et_galerkin_MMex_cn_diff
  !
 ...
 !---- 座標変数など ----
  ...
  real(8)             :: cfdx0_xmin(-km:km)     ! (0階微分@x=xmin)
  real(8)             :: cfdx1_xmin(-km:km)     ! (1階微分@x=xmin)
  real(8)             :: cfdx0_xmax(-km:km)     ! (0階微分@x=xmax)
  real(8)             :: cfdx1_xmax(-km:km)     ! (1階微分@x=xmax)
 ...
 !---------------- 座標値の設定 ---------------------
  do k=-km,km
     cfdx1_xmax(k) = 1.0D0
     cfdx0_xmax(k) = abs(2*k*Pi/XL)
     cfdx1_xmin(k) = 1.0D0
     cfdx0_xmin(k) = -abs(2*k*Pi/XL)
  enddo

  call et_galerkin_Initial(im,jm,km,lm,xmin,xmax,ymin,ymax,     &
         MMex_cfdx0_xmin=cfdx0_xmin, MMex_cfdx1_xmin=cfdx1_xmin,&
         MMex_cfdx0_xmax=cfdx0_xmax, MMex_cfdx1_xmax=cfdx1_xmax   )
   ...
   !------------------- 出力 ----------------------
    subroutine output_gtool4_init
      ...
      call HistoryCreate( &                               ! ヒストリー作成
           file='et_galerkin_MMex_cn_diff.nc', &
...
end program et_galerkin_MMex_cn_diff
