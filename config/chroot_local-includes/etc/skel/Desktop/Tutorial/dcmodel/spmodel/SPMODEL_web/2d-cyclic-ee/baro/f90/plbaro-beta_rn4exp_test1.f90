...
!            +(-1)^{p+1}\nu_{2p}\nabla^2p \nabla^2\psi
!
program plbaro_beta_rn4exp_test1
...
!== 宣言部 ============================================================
 ...
 !---- NAMELIST 変数 ----
  ...
  character(len=30),parameter  :: nmlfile='plbaro-beta_rn4exp_test1.nml'
 ...
 !
  subroutine output_restart_init
    call HistoryCreate( &
           file=trim(rst_file), &
           title=trim(title), &
           source='plbaro-beta_rn4exp_test1.f90 (2005/10/31)', &
...
end program plbaro_beta_rn4exp_test1
