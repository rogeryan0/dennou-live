...
!
!履歴  2008/10/12  竹広真一: et_galerkin_RS_cn_diff.f90 より改造
!      
!
program et_RF_cn_diff
  !
 ...
 !---- 物理パラメター ----
  ...
  character(len=2), parameter :: VelBC='RF'     ! 速度場境界条件(RR/SS/RS/SR)
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                                         ! Cn 法による時間積分
     ...
     et_Psi = at_ay(ey_Vor2Strm_ey(ay_at(et_Zeta),cond=velBC))
   ...
   !------------------- 拡散項(流線関数用) ----------------------
    subroutine CNDiffusionMatrixStrm( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      do l=0,lm
         ...
         et_DI = at_ay(ey_Vor2Strm_ey(ay_at(et_DI),cond=velBC))
