...
program wq_diff_cn_tormag_tau
 ...
 !---- 変数 ----
  real(8), dimension(0:im-1,jm,km)   :: xyr_Tormag   ! トロイダル磁場
  real(8), dimension((nm+1)**2,0:lm) :: wq_Tormag    ! トロイダル磁場
  real(8), dimension((nm+1)**2,km)   :: wr_Tormag    ! トロイダル磁場

  real(8), dimension(0:im-1,jm,km) :: xyr_TormagInit ! トロイダル磁場(初期値)
  real(8), dimension(0:im-1,jm,km) :: xyr_TormagSol  ! トロイダル磁場(解析解)

 ! Crank Nicholson 陰的計算用拡散行列(トロイダル磁場)
  real(8)             :: DifLUMT_Tormag((nm+1)*(nm+1),0:lm,0:lm)
  integer             :: kpivot_Tormag((nm+1)*(nm+1),0:lm)    ! ピボット
 ...
 !---------------- 座標値の設定 ---------------------
  ...
  call CNDiffusionMatrix( kappa, dt, DifLUMT_Tormag, kpivot_Tormag )
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                               ! Crank-Nicolson 法による時間積分
     ...
     wq_Tormag = LUSolve(DifLUMT_Tormag,kpivot_Tormag,wq_Tormag)
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    ...
    call HistoryAddVariable( &                                ! 変数定義
           ...
           longname='Toroidal magnetic potential', units='1', xtype='double')
    call HistoryAddVariable( &                                ! 変数定義
           varname='tormagsol', dims=(/'lon','lat','rad','t  '/), & 
           longname='Toroidal magnetic potential(analytic)', &
           units='1', xtype='double')
    call HistoryAddVariable( &                                ! 変数定義
           varname='tormagerror', dims=(/'lon','lat','rad','t  '/), & 
           longname='Toroidal magnetic potential(error)', &
           units='1', xtype='double')
  ...
  subroutine output_gtool4
    ...
    call HistoryPut('tormag',xyr_Tormag)
    call HistoryPut('tormagsol',xyr_TormagSol)
    call HistoryPut('tormagerror',xyr_Tormag-xyr_TormagSol)
...
end program wq_diff_cn_tormag_tau
