...
program wu_diff_cn_torvel
 ...
 !---- 変数 ----
  real(8), dimension(0:im-1,jm,0:km) :: xyr_Torvel   ! 温度
  real(8), dimension((nm+1)**2,0:lm) :: wu_Torvel    ! 温度
  real(8), dimension((nm+1)**2,0:km) :: wr_Torvel    ! 温度

  real(8), dimension(0:im-1,jm,0:km) :: xyr_TorvelInit  ! 温度(初期値)
  real(8), dimension(0:im-1,jm,0:km) :: xyr_TorvelSol   ! 温度(解析解)

 ! Crank Nicholson 陰的計算用拡散行列(温度)
  real(8)             :: DifLUMT_Torvel((nm+1)*(nm+1),0:lm,0:lm)
  integer             :: kpivot_Torvel((nm+1)*(nm+1),0:lm)    ! ピボット
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                                         ! Euler 法による時間積分
     time = it * dt
     wu_Torvel = wu_Torvel + dt/2 * wu_wr( nu * wr_Lapla_wu(wu_Torvel) )
     call wu_TorBoundaryGrid(wu_Torvel,cond='F')
     wu_Torvel = LUSolve(DifLUMT_Torvel,kpivot_Torvel,wu_Torvel)
...
99    continue
 ...
 !------------------- 拡散項 ----------------------
  subroutine CNDiffusionMatrix( Diffc, dt, DiffLUMatrix, kpivot )
    ...
    do l=0,lm
       ...
       call wu_TorBoundaryGrid(wu_DI,cond='F')
 ...
 !------------------- 出力ルーチン ----------------------
  ...
  subroutine output_gtool4
    ...
    xyr_Torvel = xyr_wr(wr_DRad_wu(wu_xyr(xyr_TorVel))) - xyr_TorVel/xyr_Rad
    write(6,*)xyr_Torvel(1,1,0)
