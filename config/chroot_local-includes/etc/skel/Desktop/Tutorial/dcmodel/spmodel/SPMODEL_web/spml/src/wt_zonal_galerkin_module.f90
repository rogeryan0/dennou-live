...
!----------------------------------------------------------------------
!
!表題  wt_zonal_galerkin_module
!
!    3 次元球殻領域(軸対称版)
!    球面調和函数展開 + チェビシェフ−ガラーキン法
!
!    spml/wt_zonal_galerkin_module モジュールは球面上および球殻内での
!    経度方向に一様な帯状的軸対称 2 次元流体運動をスペクトルガラーキン法に
!    よって数値計算するための Fortran90 関数を提供するものである. 
!
!    水平方向にルジャンドル多項式変換および上下の境界壁を扱うための
!    チェビシェフ多項式ガラーキン変換を用いる場合のスペクトル計算のための
!    さまざまな関数を提供する. 
!
!    内部で wt_zonal_module, at_a?_gakerkin_module を用いている. 
!    最下部ではルジャンドル多項式およびチェビシェフ多項式変換のエンジンとして 
!    ISPACK の Fortran77 サブルーチンを用いている.
!
!    wt_zonal_galerkin_modulde で提供される関数・サブルーチンは 3 次元的
!    流体運動を扱う wt_galerki_module モジュールで用いられているものと名前および
!    インターフェースが共通になるように設計してある. したがって, 
!    wt_galerkin_module を用いて構成された 3 次元モデルを帯状方向に一様な 
!    2 次元軸対称モデルへと改造するには次の手順が必要となる. 
!
!      * use 文での wt_galerkin_module の引用を wt_zonal_galerkin_module に変更する.
...
!      * gtool 出力の次元変数変更する. 
!
!履歴  2009/09/09  竹広 真一  wt_galerkin_module を軸対称版に改造
!      2010/03/10  佐々木洋平  threadprivate 削除(コンパイラ依存)
!             
!
!++
module wt_zonal_galerkin_module
  !
  != wt_zonal_galerkin_module
  !
  ...
  !  さまざまな関数を提供する. 
  !
  !  内部で wt_zonal_module, at_a?_gakerkin_*_module を用いている. 
  !  最下部ではルジャンドル多項式およびチェビシェフ多項式変換のエンジンとして 
  !  ISPACK の Fortran77 サブルーチンを用いている.
  !
  !  wt_zonal_galerkin_module で提供される関数・サブルーチンは 3 次元的
  !  流体運動を扱う wt_module モジュールで用いられているものと名前および
  !  インターフェースが共通になるように設計してある. したがって, 
  !  wt_galerkin_module を用いて構成された 3 次元モデルを帯状方向に一様な 
  !  2 次元軸対称モデルへと改造するには次の手順が必要となる. 
  !
  ...
  !--------------- 初期化 -----------------
    subroutine wt_galerkin_Initial(i,j,k,n,l,r_in,r_out,&
      ...
      real(8),intent(in),optional :: MMex_cfdx0_ri(n+1) ! (0階微分@r=ri)
      real(8),intent(in),optional :: MMex_cfdx1_ri(n+1) ! (1階微分@r=ri)
      real(8),intent(in),optional :: MMex_cfdx0_ro(n+1) ! (0階微分@r=ro)
      real(8),intent(in),optional :: MMex_cfdx1_ro(n+1) ! (1階微分@r=ro)
      ...
      call MessageNotify('M','wt_galerkin_initial',&
           'wt_zonal_galerkin_module (2009/09/09) is initialized')
   ...
   !----- ポロイダル速度場ポテンシャル φを ▽^2φ から求める ------
    function wq_LaplaPol2Pol_wt(wt_LaplaPolvel)
      ...
      !  ポロイダル速度場ポテンシャル φを ▽^2φ から求める
      !
      real(8), intent(IN) :: wt_LaplaPolvel(nm+1,0:lm)
        !(in) ポロイダル速度場ポテンシャルのラプラシアン ▽^2φ
      real(8)             :: wq_LaplaPol2Pol_wt(nm+1,4:lm) 
      ...
      real(8) :: wq_work(nm+1,ls:lm)
      real(8) :: wt_work(nm+1,0:lm)
      ...
      if ( first ) then
         first = .false.
         allocate(LaplaMT(nm+1,0:lm,0:lm))
         allocate(LaplaInvMT(nm+1,ls:lm,ls:lm))
         allocate(kpvot(nm+1,ls:lm))
...
end module wt_zonal_galerkin_module
