...
!表題  w_zonal_module
!
!   spml/w_zonal_module モジュールは球面上での経度方向に一様な帯状的 
!   1 次元流体運動をルジャンドル多項式を用いたスペクトル法によって
!   数値計算するための Fortran90 関数を提供する. 
!
!   内部で l_module を用いている. 最下部ではルジャンドル多項式変換
!   のエンジンとして ISPACK の Fortran77 サブルーチンを用いている.
!
!   w_zonal_modulde で提供される関数・サブルーチンは 2 次元的流体運動を扱う 
!   w_module モジュールで用いられているものと名前およびインターフェースが
!   共通になるように設計してある. したがって, w_module を用いて構成された
!   2 次元モデルを帯状方向に一様な 1 次元軸対称モデルへと改造するには
!   次の手順が必要となる. 
!
!      * use 文での w_module の引用を w_zonal_module に変更する. 
...
!
!
!履歴  2008/12/28  竹広真一  w_module より軸対称モデル用に改造
!      2009/01/08  竹広真一  w_Initial メッセージに日付を追加
!
!++
module w_zonal_module
  !
  != w_zonal_module
  !
  ! Authors:: Shin-ichi Takehiro
  ! Version:: $Id: w_zonal_module.f90,v 1.6 2009-02-28 21:33:47 uwabami Exp $
  ...
  ! 次の手順が必要となる. 
  !
  ! 1. use 文での wt_module の引用を w_zonal_module に変更する. 
  ...
  !--------------- 初期化 -----------------
    subroutine w_initial(n_in,i_in,j_in,np_in)
      ...
      call MessageNotify('M','w_initial',&
           'w_zonal_module (2009/01/08) is initialized')
