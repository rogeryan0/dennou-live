...
!----------------------------------------------------------------------
!
!表題  wt_zonal_galerkin_module_sjpack
!
!    3 次元球殻領域(軸対称版)
!    球面調和函数展開 + チェビシェフ−ガラーキン法
!
!    spml/wt_zonal_galerkin_module_sjpack モジュールは球面上および球殻内での
...
!    さまざまな関数を提供する. 
!
!    内部で wt_zonal_module_sjpack, at_a?_gakerkin_module を用いている. 
!    最下部ではルジャンドル多項式およびチェビシェフ多項式変換のエンジンとして 
!    ISPACK の Fortran77 サブルーチンを用いている.
!
!    関数, サブルーチンの名前と機能は wt_zonal_galerkin_module のものと同じである. 
!    したがって use 文を wt_zonal_galerkin_module から 
!    wt_zonal_galerkin_module_sjpack に変更するだけで SJPACK の機能が
!    使えるようになる. 
!
...
!
!履歴  2009/10/04  竹広 真一  wt_zonal_galerkin_module を軸対称版に改造             
!
!++
module wt_zonal_galerkin_module_sjpack
  !
  != wt_zonal_galerkin_module_sjpack
  !
  ...
  !  さまざまな関数を提供する. 
  !
  !  内部で wt_zonal_module_sjpack, at_a?_gakerkin_*_module を用いている. 
  !  最下部ではルジャンドル多項式およびチェビシェフ多項式変換のエンジンとして 
  !  ISPACK の Fortran77 サブルーチンを用いている.
  !
  !  関数, サブルーチンの名前と機能は wt_zonal_galerkin_module のものと同じである. 
  ...
  !--------------- 初期化 -----------------
    subroutine wt_galerkin_Initial(i,j,k,n,l,r_in,r_out,&
      ...
      call MessageNotify('M','wt_galerkin_initial',&
           'wt_zonal_galerkin_module_sjpack (2009/10/04) is initialized')
...
end module wt_zonal_galerkin_module_sjpack
