...
!履歴  2008/05/26  竹広真一
!
program w_interpolate11_mpi_test
 ...
 !---------------- MPI スタート ---------------------
  ...
  pi = atan(1.0D0)*4.0D0
  Alon = (/pi/3,-pi/4,-3*pi/5/) ; Alat = (/pi/3,pi/5,-pi/7/)

  call w_mpi_Initial( nm, im, jm )

  call MessageNotify('M','w_Interpolate11_mpi_test', &
                      'w_interpolate_module interpolate(11) tests') 
  ...
  call MessageNotify('M','w_test_Interpolate', &
                      'w_interpolate_module interpolate(11) tests succeeded!') 

 !------ MPIの終了 ------

  call MPI_FINALIZE(IERR)      

end program w_interpolate11_mpi_test
