...
!   Fortran90 関数を提供する. 
!
!   球面上の 1 層モデル用 w_module_sjpack モジュールを多層モデル用に
!   拡張したものであり, 同時に複数個のスペクトルデータ, 格子点データに
!   対する変換が行える.
!
!   wa_module_sjpack は実際には基本変換, 微分計算, 積分・平均計算, 
!   スペクトル解析をそれぞれ担っている下部モジュール wa_base_module_sjpac, 
!   wa_deriv_module_sjpack, wa_integral_module_sjpack, 
!   wa_spectrum_module_sjpack および 1 層用のモジュール w_module からなっている.
!
...
!   したがって use 文を w_module から wa_module_sjpack に
...
!   wa_module_sjpack のものは初期化したのちにしか使うことができない. 
!
...
module wa_module_sjpack
  ...
  !
  !   関数, サブルーチンの名前と機能は wa_module のものと同じである. 
  !   したがって use 文を wa_module から wa_module_sjpack に
  !   変更するだけで SJPACK の機能が使えるようになる. 
  ! 
  !   ただし l_nm, nm_l の使い方には注意されたい. wa_module の l_nm, nm_l は
  !   wa_Initial で初期化しなくとも用いることができる(結果が切断波数に依らない)が,
