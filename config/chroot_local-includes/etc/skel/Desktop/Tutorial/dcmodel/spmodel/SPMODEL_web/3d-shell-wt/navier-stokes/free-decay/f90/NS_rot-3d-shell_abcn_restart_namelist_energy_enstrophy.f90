...
!      2005/03/04  竹広 真一   エネルギー, エンストロフィー計算テスト
...
program NS_rot_3d_shell_abcn_restart_namelist
  ...
  use lumatrix
  use wt_module
  use at_module,  only : a_Int_ag, Int_g
 ...
 !---- 変数 ----
  ...
  real(8), dimension(0:km,0:nm)  :: zn_EkTor   ! 運動エネルギー(トロイダル成分)
  real(8), dimension(0:km,0:nm)  :: zn_EkPol   ! 運動エネルギー(ポロイダル成分)
 ...
 !---- その他 ----
  ...
  integer :: it=0
  real(8) :: time
  real(8), dimension((nm+1)**2,0:km)  :: wz_Data1, wz_Data2  ! 作業領域
  integer :: n, m
...
! 以下 サブルーチン
 ...
 !------------------- 初期値設定(ファイルなし) ----------------------
  subroutine set_initial_values(xyz_Torvel, xyz_Polvel )
    ...
    real(8), dimension((nm+1)**2,0:km) :: wz_Torvel       ! トロイダル速度
    real(8), dimension((nm+1)**2,0:km) :: wz_Polvel       ! ポロイダル速度

    real(8), parameter  :: pi=3.1415926535897932385D0

    real(8), parameter  :: Omega=1.0D0                   ! 回転角速度
    real(8), parameter  :: LonOmega=0.0D0  *(pi/180.0D0) ! 回転軸経度
    real(8), parameter  :: LatOmega=90.0D0 *(pi/180.0D0) ! 回転軸緯度

    ! 速度場

   ! 剛体回転場
    xyz_Torvel =  Omega * xyz_Rad &
                   * ( cos(LatOmega)*cos(xyz_Lat)*cos(LonOmega-xyz_Lon) &
                       + sin(LatOmega)*sin(xyz_Lat) )
    xyz_Polvel =  0.0D0
 ...
 !------------------- 出力 ----------------------
  subroutine output_gtool4_init
   ...
   !---- ヒストリーファイル作成 ----
    call HistoryCreate( &
           ...
           title=trim(title), &
           source='NS_rot-3d-shell_abcn_restart_namelist_energy_enstrophy.f90 (2008/08/10)', &
           institution='GFD_Dennou Club SPMODEL project',     &
           dims=(/'lon','lat','rad','n  ','t  '/), &
           dimsizes=(/im,jm,km+1,nm+1,0/),&
           longnames=(/'Longitude            ','Latitude             ',&
                       'Radius               ','Hor.total wave number',&
                       'time                 '/),&
           units=(/'1','1','1','1','1'/),   &
   ...
   !---- その他診断量定義 ----
    ...
    call HistoryAddVariable( &                                ! 変数定義
           varname='ekrad', dims=(/'rad','t  '/), & 
           longname='horizontally integrated kinetic energy', &
           units='1', xtype='double')

    call HistoryAddVariable( &                                ! 変数定義
           varname='ektor', dims=(/'rad','n  ','t  '/), & 
           longname='Toroidal kinetic energy', units='1', xtype='double')
    call HistoryAddVariable( &                                ! 変数定義
           varname='ekpol', dims=(/'rad','n  ','t  '/), & 
  ...
  subroutine output_gtool4
   ...
   !---- 診断量出力 ----
    ...
    call HistoryPut('ek',IntLonLatRad_xyz((xyz_Vlon**2+xyz_Vlat**2+xyz_Vrad**2)/2))
    call HistoryPut('ekrad',z_IntLonLat_xyz((xyz_Vlon**2+xyz_Vlat**2+xyz_Vrad**2)/2))
    call HistoryPut('um',yz_avrLon_xyz(xyz_Vlon))
    call HistoryPut('hel',xyz_Vlon*xyz_Zlon + xyz_Vlat*xyz_Zlat + xyz_Vrad*xyz_Zrad)

    wz_Data1 = wz_wt(wt_Torvel)
    do n=0,nm
       zn_EkTor(:,n) = 0.5 * n*(n+1)* (4*pi) &
            * z_Rad**2 * sum(wz_Data1(l_nm(n,(/(m,m=-n,n)/)),:)**2,1)
    enddo
    wz_Data1 = wz_wt(wt_Polvel)
    wz_Data2 = wz_wt(wt_DRad_wt(wt_Polvel))
    do n=0,nm
       zn_EkPol(:,n) = &
                 + 0.5* n*(n+1)* (4*pi) &
    ...
    call HistoryPut('ektor',zn_EkTor)
    call HistoryPut('ekpol',zn_EkPol)
    call HistoryPut('ekpot',IntRad_z(sum(zn_EkTor,2)/z_Rad**2)+IntRad_z(sum(zn_EkPol,2)/z_Rad**2))
