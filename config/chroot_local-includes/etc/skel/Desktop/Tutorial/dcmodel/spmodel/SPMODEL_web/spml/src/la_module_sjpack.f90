...
!表題  la_module
!
!   spml/la_module_sjpack モジュールは球面上での経度方向に一様な帯状的
!   子午面 2 次元流体運動をルジャンドル多項式を用いたスペクトル法によって
...
!
!   関数, サブルーチンの名前と機能は la_module のものと同じに設計してある. 
!   したがって use 文を l_module から la_module_sjpack に変更するだけで 
!   SJPACK の機能が使えるようになる. 
!
!履歴  2009/09/22  竹広真一  la_module を SJPACK 用に改造
!
!++
module la_module_sjpack
  !
  != la_module
  !
  ! Authors:: Shin-ichi Takehiro, Youhei SASAKI
  ! Version:: $Id: la_module_sjpack.f90,v 1.1 2009-09-24 07:12:09 takepiro Exp $
...
contains
  ...
  !--------------- 初期化 -----------------
    subroutine la_initial(n_in,j_in,l_init)
      ...
      call MessageNotify(&
        'M','la_initial','la_module_sjpack (2009/09/22) is initialized')
