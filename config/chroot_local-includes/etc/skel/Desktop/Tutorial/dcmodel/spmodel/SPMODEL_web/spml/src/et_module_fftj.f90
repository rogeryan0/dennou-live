...
!
!表題  et_module_fftj
!      2 次元水路領域問題, Fourier 展開 + Chebyshev 展開法(fftj 版)
!
!      spml/et_module_fftj モジュールは 2 次元水路領域での流体運動を
!      スペクトル法により数値計算を実行するための Fortran90 関数を提供する. 
!      周期的な境界条件を扱うための X 方向へのフーリエ変換と境界壁を扱うための
!      Y 方向のチェビシェフ変換を用いる場合のスペクトル計算のためのさまざまな
!      関数を提供する. 
!
!      内部で ae_module_fftj, at_module を用いている. 
!      最下部ではフーリエ変換のエンジンとして fftj, 
!      チェビシェフ変換のエンジンとして ISPACK/FTPACK の
!      Fortran77 サブルーチンを用いている.
!
!履歴  2009/09/10  竹広真一  et_module.f90 を fftj 用に改造
!      2010/03/10  佐々木洋平  threadprivate 削除(コンパイラ依存)
!
!++
module et_module_fftj
  !
  != et_module_fftj
  !
  ! Authors:: Shin-ichi Takehiro, Youhei SASAKI
  ! Version:: $Id: et_module_fftj.f90,v 1.3 2010-03-02 10:09:05 uwabami Exp $
  ...
  use dc_message
  use lumatrix
  use ae_module_fftj, x_X => g_X, x_X_weight => g_X_Weight, &
                 e_x => e_g, ae_ax => ae_ag, &
                 x_e => g_e, ax_ae => ag_ae
  ...
  !--------------- 初期化 -----------------
    subroutine et_Initial(i,j,k,l,xmin,xmax,ymin,ymax)
      ...
      call MessageNotify('M','et_initial',&
           'et_module_fftj (2009/09/10) is initialized')
