...
!履歴  2008/05/26  竹広真一
!
program w_interpolate10_mpi_test
 ...
 !---------------- MPI スタート ---------------------
  ...
  pi = atan(1.0D0)*4.0D0
  Alon = (/pi/3,-pi/4/) ; Alat = pi/5

  call w_mpi_Initial( nm, im, jm )

  call MessageNotify('M','w_interpolate10_mpi_test', &
                        'w_interpolate_module interpolate(10) tests') 
  ...
  call MessageNotify('M','w_test_Interpolate', &
                     'w_interpolate_module interpolate(10) tests succeeded!') 

 !------ MPIの終了 ------

  call MPI_FINALIZE(IERR)      

end program w_interpolate10_mpi_test
