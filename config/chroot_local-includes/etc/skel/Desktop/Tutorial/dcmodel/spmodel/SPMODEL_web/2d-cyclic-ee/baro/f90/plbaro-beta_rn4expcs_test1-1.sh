#!/usr/bin/env bash
#
# plbaro_rn4expcs_test1-1.f90 用実行シェルスクリプト
#
...
#  integer :: jm                            ! Y 方向格子点数 (>3*lm)
 ...
 &gridset km=42, lm=21, im=128, jm=64 /
...
# integer :: nstep            ! 時間積分ステップ数
 ...
 &tint    delta_t= 1.0d-01,  nstep= 100       /
...
#  integer            :: hst_intstep        ! ヒストリー出力間隔ステップ数
 ...
 &history  hst_file='$OUTPUTFILE', hst_intstep= 2, title='$TITLE' /
...
#  real(8)            :: HVisc              ! 超粘性係数
 ...
 &physics XLength=2.0D0, YLength=1.0D0, Beta=1.0, HVOrder=2, HVisc=1.0D-8 /
...
# real(8) :: Amplitude            ! 振幅
 ...
 &initwave KInitial=1, LInitial=2, Amplitude=1.0D0 /
