...
!
 ...
 !---- 変数 ----
    ...
    real(8)            :: yx_QH1(0:jm,0:im-1)    ! 格子データ(正の熱源分布)
    real(8)            :: yx_QH2(0:jm,0:im-1)    ! 格子データ(負の熱源分布)
 ...
 !---- 物理パラメター ----
    ...
    real(8), parameter :: Q1=1.0                ! 熱源分布の振幅
    real(8), parameter :: A1=0.2                ! 熱源分布の Y 方向の幅
    real(8), parameter :: Xrange1=20.0          ! 熱源分布の X 方向の幅
    real(8), parameter :: X1=0.0                ! 熱源分布の中心

    real(8), parameter :: Q2=-1.0               ! 熱源分布の振幅
    real(8), parameter :: A2=0.2                ! 熱源分布の Y 方向の幅
    real(8), parameter :: Xrange2=20.0          ! 熱源分布の X 方向の幅
    real(8), parameter :: X2=-30.0              ! 熱源分布の中心

 !---- 時間積分パラメター ----
    real(8), parameter :: dt=2e-1               ! 時間ステップ間隔        
    integer, parameter :: nt=3000, ndisp=100    ! 時間積分数, 表示ステップ
    ...
    do i=0,im-1
       do j=0,jm
          yx_X(j,i)= (xmax-xmin)/im*i + xmin
          yx_Y(j,i)= (ymax-ymin)/jm*j + ymin
       enddo
    enddo
...
!    yx_H=transpose(yx_data) ; ec_H = ec_yx(yx_H)
 ...
 !----------------------- 熱強制分布 --------------------------
    do i=0,im-1
       do j=0,jm
          if ( abs((yx_X(j,i)-X1)) .le. Xrange1 ) then
             yx_QH1(j,i) = Q1 * exp( -yx_Y(j,i)**2/A1 )&
                             * cos(pi*(yx_X(j,i)-X1)/(2*Xrange1))
          else
             yx_QH1(j,i) = 0.0
          endif
       enddo
    ...
    do i=0,im-1
       do j=0,jm
          if ( abs((yx_X(j,i)-X2)) .le. Xrange2 ) then
             yx_QH2(j,i) = Q2 * exp( -yx_Y(j,i)**2/A2 )&
                             * cos(pi*(yx_X(j,i)-X2)/(2*Xrange2))
          else
             yx_QH2(j,i) = 0.0
          endif
       enddo
  ...
  contains
   ...
   !----------------------- gtool4 設定 ----------------------------
    subroutine output_gtool4_init
      ...
      call HistoryAddVariable( &                                 ! 変数定義
           varname='q', dims=(/'x','y'/), & 
           longname='mass source', units='1/s', xtype='double')
      call HistoryPut('q',real(transpose(yx_ec(ec_QH))))
