...
!----------------------------------------------------------------------
!
!表題  tee_module テストプログラム (境界値問題, ポロイダル磁場, Tau 法)
!
!履歴  2009/12/19  竹広真一
!
program tee_module_polmagbctau_test
 ...
 !---- 空間解像度設定 ----
  ...
  integer, parameter :: lm=10, mm=21, nm=10       ! 切断波数の設定(X,Y)

 !---- 変数 ----
  real(8)            :: zyx_Polmag(0:km,0:jm-1,0:im-1)      ! 格子データ

  real(8)            :: tee_Polmag(0:nm,-mm:mm,-lm:lm)      ! スペクトルデータ
  real(8)            :: tee_Polmag_orig(0:nm,-mm:mm,-lm:lm) ! スペクトルデータ
 ...
 !---- 座標変数など ----
  ...
  call MessageNotify('M','tee_module_PolmagbcTau_test', &
...
end program tee_module_polmagbctau_test
