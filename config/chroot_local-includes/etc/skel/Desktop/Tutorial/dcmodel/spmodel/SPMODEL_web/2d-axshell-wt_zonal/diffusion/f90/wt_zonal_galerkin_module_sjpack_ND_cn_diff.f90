...
!      外側でフラックス固定条件(dT/dr=0), 内側で温度固定条件(T=0)
!
...
!
!履歴  2009/10/04  竹広真一 wt_zonal_galerkin_ND_cn_diff.f90 より sjpack 用に改造
!
program wt_zonal_galerkin_module_sjpack_ND_cn_diff
  !
 ...
 !---- 変数 ----
  ...
  real(8), dimension(nm+1,2:lm)      :: wb_Temp    ! 温度
 ...
 !---------------- 座標値の設定 ---------------------
  call wt_galerkin_Initial(im,jm,km,nm,lm,ri,ro,ND=.true.)

  call CNDiffusionMatrixND( kappa, dt, DifLUMT_Temp, kpivot_Temp )
 ...
 !------------------- 時間積分 ----------------------
  do it=1,nt                                         ! Euler 法による時間積分
     time = it * dt
     wb_Temp = wb_Temp + dt/2 * wb_wt( kappa * wt_Lapla_wt(wt_wb(wb_Temp)) )
     wb_Temp = LUSolve(DifLUMT_Temp,kpivot_Temp,wb_Temp)
...
contains
  ...
  function det(l,x)
    !
    ! 係数行列式の計算
    ! Δ=sbj(ri*x)sbn1(ro*x)-sbn(ri*x)sbj1(ro*x)
    !
    integer, intent(IN) :: l                 ! 次数
    real(8), intent(IN) :: x                 ! 
    real(8)             :: det               ! 行列式

    det=x*sbj(l,ri*x)*sbn1(l,ro*x) - x*sbn(l,ri*x)*sbj1(l,ro*x)

  end function det

  function CalAlpha(l,n)
    !
    ! sbj(ri*x)sbn1(ro*x)-sbn(ri*x)sbj1(ro*x) の解を求める
    !
    integer, intent(IN) :: l                 ! 次数
...
99  continue
   ...
   !------------------- 拡散項 ----------------------
    subroutine CNDiffusionMatrixND( Diffc, dt, DiffLUMatrix, kpivot )
      ...
      real(8)               :: wb_I(nm+1,2:lm)       ! 作業用変数

      integer :: l

      DiffLUMatrix = 0.0
      do l=2,lm
         wb_I = 0.0 ; wb_I(:,l) = 1.0             ! 各波数成分独立
         DiffLUMatrix(:,:,l) &
              = wb_I - Diffc * dt/2.0 * wb_wt(wt_Lapla_wt(wt_wb(wb_I)))
      enddo
 ...
 !------------------- 出力ルーチン ----------------------
  subroutine output_gtool4_init
    ...
    call HistoryCreate( &                                  ! ヒストリー作成
           file='wt_zonal_galerkin_module_sjpack_ND_cn_diff.nc', &
...
end program wt_zonal_galerkin_module_sjpack_ND_cn_diff
