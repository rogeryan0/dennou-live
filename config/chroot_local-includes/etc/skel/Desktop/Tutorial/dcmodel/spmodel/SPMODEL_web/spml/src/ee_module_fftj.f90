...
!      提供する. 
!
!      内部で ISPACK/PZPACK(fftj) の Fortran77 サブルーチンを呼んでいる. 
!      スペクトルデータおよび格子点データの格納方法については
!      ISPACK/PZPACK のマニュアルを参照されたい. 
!
!履歴  2009/09/09  竹広真一  ee_module.f90 を pzpack(fftj) 用に改造
!
!++
module ee_module_fftj
  !
  != ee_module_fftj
  !
  ! Authors:: Shin-ichi Takehiro, Youhei SASAKI
  ! Version:: $Id: ee_module_fftj.f90,v 1.2 2009-09-12 12:38:33 takepiro Exp $
  ...
  integer, parameter :: ind2imax=11              ! Max(im) = 2048 = 2^{11}
  integer, parameter :: ind2jmax=11              ! Max(jm) = 2048 = 2^{11}
  ...
  !--------------- 初期化 -----------------
    subroutine ee_Initial(i,j,k,l,xmin,xmax,ymin,ymax,id)
      ...
      ind2i = 1
      do while ( im /= 2**ind2i .AND. ind2i <= ind2imax ) 
         ind2i=ind2i+1
      enddo
      if ( ind2i > ind2imax ) then
         call MessageNotify('E','ee_Initial', &
              'Number of X grid points IM should be 2^N and <= 2048')
      else if ( km <= 0 ) then
         call MessageNotify('E','ee_Initial', &
              'Number of waves kM should be positive')
      elseif ( km >= im/2 ) then
         call MessageNotify('E','ee_Initial', &
      ...
      ind2j = 1
      do while ( jm /= 2**ind2j .AND. ind2j <= ind2jmax ) 
         ind2j=ind2j+1
      enddo
      if ( ind2j > ind2jmax ) then
         call MessageNotify('E','ee_Initial', &
              'Number of Y grid points JM should be 2^N and <= 2048')
      else if ( lm <= 0 ) then
         call MessageNotify('E','ee_Initial', &
              'Number of waves LM should be positive')
      elseif ( lm >= jm/2 ) then
         call MessageNotify('E','ee_Initial', &
              'LM shoud be less than JM/2')
      endif
      ...
      allocate(params(nparams)%itj(4))
      allocate(params(nparams)%iti(4))
      allocate(params(nparams)%tj(jm*6))
      allocate(params(nparams)%ti(im*8))
      ...
      call MessageNotify('M','ee_initial','ee_module_fftj (2009/09/09) is initialized')
  ...
  !--------------- 基本変換 -----------------
    function yx_ee(ee)
      ...
      call pzs2ga(lm,km,jm,im,ee,yx_ee,w,itj,tj,iti,ti)
    end function yx_ee
    ...
    function ee_yx(yx)
      ...
      yx_tmp = yx
      call pzg2sa(lm,km,jm,im,yx_tmp,ee_yx,w,itj,tj,iti,ti)
  ...
  !--------------- 微分計算 -----------------
    ...
    function ee_Jacobian_ee_ee(ee_a,ee_b)
      ...
      call pzajcb(lm,km,jm,im,ee_a,ee_b,ee_Jacobian_ee_ee,ws,wg,itj,tj,iti,ti)
    ...
    function ee_JacobianZ_ee(ee_zeta)
      ...
      call pzajbs(lm,km,jm,im,yl/xl,ee_Zeta,ee_JacobianZ_ee,ws,wg,itj,tj,iti,ti)
